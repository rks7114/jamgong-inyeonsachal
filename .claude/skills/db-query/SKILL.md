...
- **불회사 오탐은 수정됨(2026-08-29)**: `회사(?!불)` → `(?<!불)회사` 패턴 교정으로 불회사(佛會寺, 나주) 3개 레코드가 비사찰 목록·엔진 제외 대상에서 해제됐다 (237→234건). 그래도 남은 목록에 화엄사 사찰음식체험관 같은 정탐과 또 다른 오탐이 섞여 있을 수 있으니, 정리 작업 시 기계적 삭제 금지 — 반드시 육안 검토할 것. 패턴 변경은 엔진과 스크립트 2개(db-quality-check, db-query)에 동시 적용해야 한다.
