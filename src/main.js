...
// ── 인연 길잡이 챗봇 UI (render 완료 후 안전하게 초기화) ──
(function initChatbot() {
  if (document.getElementById('chatbot-wrap')) return;
  var wrap = document.createElement('div');
  wrap.id = 'chatbot-wrap';
  var style = document.createElement('style');
  style.textContent = '#chatbot-wrap{position:fixed;bottom:24px;right:20px;z-index:9000;font-family:inherit}'
    ...
    + '.cmsg{max-width:85%;padding:10px 13px;border-radius:14px;font-size:13px;line-height:1.6}'
    + '.cmsg.bot{background:rgba(0,180,255,0.12);color:#cce6ff;border-bottom-left-radius:4px;align-self:flex-start}'
    + '.cmsg.user{background:rgba(0,95,163,0.5);color:#fff;border-bottom-right-radius:4px;align-self:flex-end}'
    + '#chat-input-row{display:flex;gap:8px;padding:10px 12px;border-top:1px solid rgba(255,255,255,0.08)}'
    + '#chat-inp{flex:1;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);border-radius:10px;color:#fff;font-size:13px;padding:8px 12px;outline:none;font-family:inherit}'
    + '#chat-snd{background:linear-gradient(135deg,#005fa3,#00b4d8);border:none;color:#fff;border-radius:10px;padding:8px 14px;font-size:13px;cursor:pointer;font-weight:700}'
    + '#chat-snd:hover{opacity:0.85}';
  document.head.appendChild(style);
  wrap.innerHTML = '<div id="chat-panel">'
    + '<div id="chat-header"><span>🪷 인연 길잡이</span><button id="chat-close">✕</button></div>'
    + '<div id="chat-messages"><div class="cmsg bot">안녕하세요! 인연 길잡이예요 😊<br>사찰이나 사주팔자에 대해 무엇이든 물어보세요.</div></div>'
    + '<div id="chat-input-row"><input id="chat-inp" type="text" placeholder="메시지를 입력하세요..." /><button id="chat-snd">전송</button></div>'
    + '</div>'
  ...
  var snd = document.getElementById('chat-snd');
  var history = [];
  ...
  async function send() {
    ...
    snd.disabled = true;
    try {
      var body = { messages: history.slice(-12) };
      if (_sajuContext) body.sajuContext = _sajuContext;
      var r = await fetch('/api/chatbot', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body) });
      var d = await r.json();
      var reply = d.reply || '잠시 연결이 어렵습니다.';
      loading.textContent = reply;
      history.push({ role: 'assistant', content: reply });
    } catch(e) {
      loading.textContent = '연결 오류가 발생했습니다.';
    } finally {
      snd.disabled = false;
      msgs.scrollTop = msgs.scrollHeight;
    }
