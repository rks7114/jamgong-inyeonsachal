...
생년월일을 다시 확인해주세요.`);return}y.purpose=e,Pe(y)}catch(G){clearInterval(xe),I.classList.add("hidden"),I.innerHTML="",D&&(D.style.display=""),alert(`매칭 오류: ${G.message}`)}finally{k.disabled=!1,k.textContent="인연사찰 찾기"}}})}function rt(e,a){const f=e.eightChar;if(!f)return"";const A=o=>(o==null?void 0:o[0])||"",E=o=>(o==null?void 0:o[1])||"",p=A(f.day),m=A(f.month),$=A(f.year),n=A(f.time),t=E(f.day),z=E(f.month),w=E(f.year),l=E(f.time),j=[l,t,z,w].filter(Boolean),S={甲:{甲:"비견",乙:"겁재",丙:"식신",丁:"상관",戊:"편재",己:"정재",庚:"편관",辛:"정관",壬:"편인",癸:"정인"},乙:{甲:"겁재",乙:"비견",丙:"상관",丁:"식신",戊:"정재",己:"편재",庚:"정관",辛:"편관",壬:"정인",癸:"편인"},丙:{甲:"편인",乙:"정인",丙:"비견",丁:"겁재",戊:"식신",己:"상관",庚:"편재",辛:"정재",壬:"편관",癸:"정관"},丁:{甲:"정인",乙:"편인",丙:"겁재",丁:"비견",戊:"상관",己:"식신",庚:"정재",辛:"편재",壬:"정관",癸:"편관"},戊:{甲:"편관",乙:"정관",丙:"편인",丁:"정인",戊:"비견",己:"겁재",庚:"식신",辛:"상관",壬:"편재",癸:"정재"},己:{甲:"정관",乙:"편관",丙:"정인",丁:"편인",戊:"겁재",己:"비견",庚:"상관",辛:"식신",壬:"정재",癸:"편재"},庚:{甲:"편재",乙:"정재",丙:"편관",丁:"정관",戊:"편인",己:"정인",庚:"비견",辛:"겁재",壬:"식신",癸:"상관"},辛:{甲:"정재",乙:"편재",丙:"정관",丁:"편관",戊:"정인",己:"편인",庚:"겁재",辛:"비견",壬:"상관",癸:"식신"},壬:{甲:"식신",乙:"상관",丙:"편재",丁:"정재",戊:"편관",己:"정관",庚:"편인",辛:"정인",壬:"비견",癸:"겁재"},癸:{甲:"상관",乙:"식신",丙:"정재",丁:"편재",戊:"정관",己:"편관",庚:"정인",辛:"편인",壬:"겁재",癸:"비견"}},W={비견:"#00D2FF",겁재:"#6B82F9",식신:"#4CAF50",상관:"#81C784",편재:"#FF9800",정재:"#FFB74D",편관:"#F44336",정관:"#EF9A9A",편인:"#CE93D8",정인:"#BA68C8"},oe={비견:"독립심·경쟁심",겁재:"추진력·욕망",식신:"표현력·식복",상관:"재능·자유",편재:"사업·활동성",정재:"안정적 재물",편관:"권력·결단",정관:"명예·원칙",편인:"직관·학문",정인:"학습·보호"},b=o=>p&&S[p]&&S[p][o]||"",k=[{label:"시(時)",gan:n,ji:l,ss:b(n)},{label:"일(日)",gan:p,ji:t,ss:"일원(日元)"},{label:"월(月)",gan:m,ji:z,ss:b(m)},{label:"년(年)",gan:$,ji:w,ss:b($)}],_={};k.forEach(o=>{o.ss&&o.ss!=="일원(日元)"&&(_[o.ss]=(_[o.ss]||0)+1)});const L=`
  ...
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;">
      ${[{label:"시(時)",ji:l},{label:"일(日)",ji:t},{label:"월(月)",ji:z},{label:"년(年)",ji:w}].map(o=>{const ye=y[o.ji]||[];return`<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:10px 8px;text-align:center;">
          ...
          ${ye.map((ze,Se)=>{const Me=pe[ze]||"토",Oe=ge[Me]||"#aaa",Ie=b(ze),De=Se===ye.length-1?"정기(正氣)":ne[Se]||"";return`<div style="margin-bottom:5px;">
  ...
  </div>`,P=[n,p,m,$].filter(Boolean),N=[l,t,z,w].filter(Boolean),de=[{a:"甲",b:"己",result:"土합",desc:"갑기합(甲己合) — 리더십(木)과 포용력(土)이 결합, 안정적 추진력을 발휘합니다. 공직·기업 경영에 이상적입니다."},{a:"乙",b:"庚",result:"金합",desc:"을경합(乙庚合) — 유연함(木)과 단호함(金)이 결합, 협상력과 결단력을 동시에 갖춥니다."},{a:"丙",b:"辛",result:"水합",desc:"병신합(丙辛合) — 열정(火)과 예리함(金)이 수(水)로 변환, 전략적 분석력과 깊은 통찰이 생깁니다."},{a:"丁",b:"壬",result:"木합",desc:"정임합(丁壬合) — 감수성(火)과 지혜(水)가 목(木)으로 변환, 창의적 영감과 인문학적 재능이 발달합니다."},{a:"戊",b:"癸",result:"火합",desc:"무계합(戊癸合) — 현실감(土)과 직관(水)이 화(火)로 변환, 열정적 행동력과 카리스마가 형성됩니다."}].filter(o=>P.includes(o.a)&&P.includes(o.b)),je=[{group:["申","子","辰"],result:"水局",desc:"수(水) 기운이 강하게 형성"},{group:["巳","酉","丑"],result:"金局",desc:"금(金) 기운이 강하게 형성"},{group:["寅","午","戌"],result:"火局",desc:"화(火) 기운이 강하게 형성"},{group:["亥","卯","未"],result:"木局",desc:"목(木) 기운이 강하게 형성"}].map(o=>({cnt:o.group.filter(ze=>N.includes(ze)).length,...o})).filter(o=>o.cnt>=2),fe=[{a:"子",b:"丑",result:"土합",desc:"자축합(子丑合) — 수(水)와 토(土)가 결합해 단단한 땅을 형성합니다. 신용·자산 기반이 흔들림 없이 지켜지는 강한 결속력입니다."},{a:"寅",b:"亥",result:"木합",desc:"인해합(寅亥合) — 목(木) 기운이 강화됩니다. 시작과 성장의 에너지가 서로를 북돋아 새로운 도전에 유리합니다."},{a:"卯",b:"戌",result:"火합",desc:"묘술합(卯戌合) — 화(火) 기운으로 변합니다. 따뜻한 정(情)과 열정이 관계를 불태우지만, 집착이 될 수 있어 균형이 필요합니다."},{a:"辰",b:"酉",result:"金합",desc:"진유합(辰酉合) — 금(金) 기운이 강해집니다. 현실적 판단력과 재물 결실이 단단하게 쌓이는 실속형 결합입니다."},{a:"巳",b:"申",result:"水합",desc:"사신합(巳申合) — 수(水) 기운으로 변합니다. 지혜·전략·소통이 강화되나, 화(火)와 금(金)의 긴장이 내재합니다."},{a:"午",b:"未",result:"土합",desc:"오미합(午未合) — 토(土) 기운이 두터워집니다. 중심을 잡아주는 안정 에너지로, 중년 이후 자산 축적에 유리합니다."}].filter(o=>N.includes(o.a)&&N.includes(o.b)),$e=[{a:"子",b:"午",desc:"子午충 — 수화(水火) 충돌, 변동·이동수 강함"},{a:"丑",b:"未",desc:"丑未충 — 토토(土土) 충돌, 직장·부동산 변동"},{a:"寅",b:"申",desc:"寅申충 — 목금(木金) 충돌, 충동적 행동 주의"},{a:"卯",b:"酉",desc:"卯酉충 — 목금(木金) 충돌, 이성 관계·인간관계 마찰"},{a:"辰",b:"戌",desc:"辰戌충 — 토토(土土) 충돌, 기반·환경의 급변"},{a:"巳",b:"亥",desc:"巳亥충 — 화수(火水) 충돌, 사업·건강의 기복"}].filter(o=>N.includes(o.a)&&N.includes(o.b)),ke=`
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="margin-bottom:12px;">
      ...
      ${je.map(o=>`<div style="background:rgba(76,175,80,0.05);border:1px solid rgba(76,175,80,0.18);border-radius:10px;padding:10px 12px;margin-bottom:6px;font-size:12px;color:rgba(255,255,255,0.75);">
        <span style="font-size:13px;font-weight:800;color:#AED581;margin-right:8px;">${o.group.filter(ye=>N.includes(ye)).join("·")} ${o.cnt===2?"반삼합":"삼합"}</span>${o.desc} <span style="color:rgba(255,255,255,0.4);">(→ ${o.result})</span>
    ...
    <div style="margin-bottom:12px;">
      ...
      ${fe.map(o=>`<div style="background:rgba(156,39,176,0.05);border:1px solid rgba(156,39,176,0.18);border-radius:10px;padding:12px;margin-bottom:6px;">
    ...
    ${de.length===0&&je.length===0&&fe.length===0&&$e.length===0?'<div style="font-size:12px;color:rgba(255,255,255,0.35);padding:8px 0;">원국 내 주요 합·충이 없습니다 — 대운·세운에서 형성될 때 주목하세요.</div>':""}
    <div style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);">💡 2026 丙午년: 午를 포함한 寅午戌 화국(火局) 가능성 체크</div>
  </div>`,Be=[{a:"子",b:"未",desc:"자(子)–미(未) 원진 — 서로 미워하고 불편한 관계"},{a:"丑",b:"午",desc:"축(丑)–오(午) 원진 — 의리 있지만 갈등 반복"},{a:"寅",b:"酉",desc:"인(寅)–유(酉) 원진 — 시작은 좋지만 결국 어긋남"},{a:"卯",b:"申",desc:"묘(卯)–신(申) 원진 — 예민함과 강함의 충돌"},{a:"辰",b:"亥",desc:"진(辰)–해(亥) 원진 — 마음은 맞지만 행동이 어긋남"},{a:"巳",b:"戌",desc:"사(巳)–술(戌) 원진 — 가까울수록 상처받기 쉬움"}],K=[{a:"子",b:"酉",desc:"자–유 귀문 — 강한 집착·예민한 감수성"},{a:"丑",b:"午",desc:"축–오 귀문 — 감정 기복·강박적 성향"},{a:"寅",b:"亥",desc:"인–해 귀문 — 신비로운 직관, 과몰입 주의"},{a:"卯",b:"申",desc:"묘–신 귀문 — 예민함·과한 상상력"},{a:"辰",b:"巳",desc:"진–사 귀문 — 의심·불안감"},{a:"未",b:"戌",desc:"미–술 귀문 — 우울·집착 경향"}],be=Be.filter(o=>N.includes(o.a)&&N.includes(o.b)),Fe=K.filter(o=>N.includes(o.a)&&N.includes(o.b)),Ce=`
  <div class="saju-card" style="margin-bottom:16px;">
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:flex;gap:12px;align-items:flex-start;">
      ...
      <div style="flex:1;">
        ${l.map((K,be)=>`<div style="display:flex;gap:8px;align-items:flex-start;${be<l.length-1?"margin-bottom:10px":""}"><span style="color:#00d2ff;font-size:13px;flex-shrink:0;margin-top:2px;">${["①","②","③"][be]||"✦"}</span><span style="font-size:13px;color:rgba(255,255,255,0.82);line-height:1.75;">${K}</span></div>`).join("")}
      </div>
    </div>
  </div>`,ge=`
  <div class="saju-card" style="${B}">
    ...
    <div style="margin-bottom:12px;">${oe.jobs.map(K=>ne(K,"rgba(76,175,80,0.1)","rgba(76,175,80,0.3)","#81C784")).join("")}</div>
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      <div style="background:rgba(0,200,100,0.05);border:1px solid rgba(0,200,100,0.2);border-radius:12px;padding:14px;">
        ...
        ${F.do_.map(K=>`<div style="display:flex;gap:6px;align-items:flex-start;margin-bottom:8px;"><span style="color:#69F0AE;flex-shrink:0;font-size:11px;margin-top:2px;">▸</span><span style="font-size:12px;color:rgba(255,255,255,0.78);line-height:1.65;">${K}</span></div>`).join("")}
      </div>
      <div style="background:rgba(255,80,80,0.05);border:1px solid rgba(255,80,80,0.2);border-radius:12px;padding:14px;">
        ...
        ${F.dont.map(K=>`<div style="display:flex;gap:6px;align-items:flex-start;margin-bottom:8px;"><span style="color:#FF8A80;flex-shrink:0;font-size:11px;margin-top:2px;">▸</span><span style="font-size:12px;color:rgba(255,255,255,0.78);line-height:1.65;">${K}</span></div>`).join("")}
      </div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:10px;">
      <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px;">
          ${v.colors.map(K=>`<span style="background:rgba(255,213,79,0.1);border:1px solid rgba(255,213,79,0.3);border-radius:20px;padding:4px 12px;font-size:12px;color:#FFD54F;font-weight:600;">${K}</span>`).join("")}
        </div>
  ...
  </div>`:"",Y=((Be=e.daYun)==null?void 0:Be.list)||[],X=Y.findIndex(K=>K.isCurrent),U=Y[X],q=X>=0?Y[X+1]:null,H=q?q.startAge:null,le=H?H-2:null,P=H?H+2:null,N=new Date().getFullYear(),ie=(a==null?void 0:a.year)||N,de=N-ie,ve=H!==null&&Math.abs(de-H)<=3,je=H?`
  <div class="saju-card" style="${B}">
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;">
      <div style="background:linear-gradient(135deg,rgba(0,210,255,0.06),rgba(107,130,249,0.04));border:1px solid rgba(0,210,255,0.2);border-radius:12px;padding:14px;">
        ...
        <div style="font-size:18px;font-weight:900;color:#00D2FF;margin-bottom:6px;letter-spacing:1px;">${fe.guiIn.jilju}</div>
        <div style="font-size:12px;color:rgba(255,255,255,0.72);line-height:1.8;">${fe.guiIn.reason}</div>
      </div>
      <div style="background:linear-gradient(135deg,rgba(105,240,174,0.06),rgba(0,200,83,0.04));border:1px solid rgba(105,240,174,0.2);border-radius:12px;padding:14px;">
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.72);line-height:1.8;">${fe.energy.desc}</div>
  ...
  </div>`:"";return pe+ge+ue+se+me+s+d+g+je+ce}function We(e,a,f,A){var Z,D,I,i,ee,re,xe,G;window._gunghamContext=null,window._sajuContext={eightChar:e.eightChar,distribution:e.distribution,weak:e.weak,daYun:e.daYun,samjae:e.samjae,birthInput:a,temples:((Z=f==null?void 0:f.results)==null?void 0:Z.slice(0,3).map(c=>{var h,F;return{name:(h=c.temple)==null?void 0:h.name,address:(F=c.temple)==null?void 0:F.address}}))||[]};const E=document.getElementById("results");E.classList.remove("hidden");const p=document.getElementById("match-form");p&&(p.style.display="none");const m={木:"목",火:"화",土:"토",金:"금",水:"수"},$={목:"#4CAF50",화:"#FF5722",토:"#FF9800",금:"#9E9E9E",수:"#2196F3"},n={목:"木 목",화:"火 화",토:"土 토",금:"金 금",수:"水 수"},t=e.eightChar,z=t?(()=>{const c=[{label:"시(時)",char:t.time,wx:t.timeWx},{label:"일(日)",char:t.day,wx:t.dayWx},{label:"월(月)",char:t.month,wx:t.monthWx},{label:"년(年)",char:t.year,wx:t.yearWx}],h=y=>y?y[0]:"",F=y=>y?y[1]:"",B=c.map(y=>{const ne=[...y.wx||""].map(ge=>m[ge]).filter(Boolean),pe=$[ne[0]]||"#00d2ff";return{chars:ne,col:pe}});return`
    <table class="saju-table">
      ...
      <tbody>
        ...
        <tr class="saju-table-ji">
          ${c.map((y,ne)=>`<td style="color:${B[ne].col}">${F(y.char)}</td>`).join("")}
        </tr>
  ...
  `).join(""),j=a.calendarType==="lunar"?"음력":"양력",S=`${a.hour}시 ${a.minute>0?a.minute+"분":""}`.trim();a.birthLongitude&&`${a.birthLongitude}`;const W=((D=e.weak)==null?void 0:D.부족오행)??"",oe={목:"#4CAF50",화:"#FF5722",토:"#FF9800",금:"#9E9E9E",수:"#2196F3"},b={목:"木",화:"火",토:"土",금:"金",수:"水"},k=(f==null?void 0:f.results)||[],_=k.length>0?`
...
나무관세음보살.`}}[L]:null;if(B){const s=B(m.name||"이 사찰"),d=j.gido;f?(i+='<div class="ds-card" style="background:rgba('+d.r+",0.07);border:1px solid rgba("+d.r+",0.25);border-left:4px solid "+d.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+d.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+d.r+',0.2);">🕯️ 맞춤 기도문</div>',i+='<div style="font-size:14px;color:#E2E8F0;line-height:2.1;white-space:pre-line;background:rgba(0,0,0,0.2);border-radius:12px;padding:16px;">'+s+"</div>",i+='<button id="gido-copy-btn" style="margin-top:14px;display:inline-flex;align-items:center;gap:6px;background:rgba('+d.r+",0.15);border:1px solid rgba("+d.r+",0.4);color:"+d.c+';border-radius:14px;padding:9px 20px;font-size:13px;font-weight:700;cursor:pointer;">📋 기도문 복사</button>',i+='<textarea id="gido-raw" style="position:absolute;left:-9999px;opacity:0;" readonly>'+s+"</textarea>",i+="</div>"):(i+='<div class="ds-card" style="background:rgba('+d.r+",0.07);border:1px solid rgba("+d.r+",0.25);border-left:4px solid "+d.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+d.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+d.r+',0.2);">🕯️ 맞춤 기도문</div>',i+='<div style="font-size:14px;color:#94A3B8;line-height:2;background:rgba(0,0,0,0.2);border-radius:12px;padding:16px;filter:blur(2px);pointer-events:none;">'+s.slice(0,80)+"…</div>",i+='<div style="margin-top:14px;text-align:center;font-size:15px;font-weight:800;color:#1A1200;background:#FCD34D;border-radius:10px;padding:12px 16px;letter-spacing:.03em;">🔒 전체 기도문은 멤버십 전용입니다</div>',i+="</div>")}if(i+='<div id="history-section">'+(T?'<div class="ds-card" style="background:rgba(217,119,6,0.06);border:1px solid rgba(217,119,6,0.22);border-left:4px solid #D97706;border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;"><div style="font-size:14px;font-weight:800;color:#D97706;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(217,119,6,0.2);">📜 유래 · 연혁</div><div id="history-body" style="font-size:14px;color:#E2E8F0;line-height:1.9;"><p style="margin:0;">'+te+"</p></div></div>":"")+"</div>",u.length>0){const s=j.date,d=["일","월","화","수","목","금","토"],x=u.slice(0,V).map(function(M){const v=M.date.split("-"),g=parseInt(v[1]),Y=parseInt(v[2]),X=new Date(parseInt(v[0]),g-1,Y).getDay(),U=d[X],q=X===0||X===6,H=M.dayOhaeng&&W[M.dayOhaeng[0]]||"✶",le=M.dayOhaeng&&t[M.dayOhaeng[0]]||t.금;return'<div class="detail-date-card" style="display:inline-flex;flex-direction:column;align-items:center;gap:5px;background:rgba('+le.r+",0.1);border:1px solid rgba("+le.r+',0.35);border-radius:16px;padding:14px 18px;margin:4px;min-width:74px;"><span style="font-size:20px;">'+H+'</span><span style="font-size:17px;font-weight:900;color:'+le.c+';">'+g+"/"+Y+'</span><span style="font-size:12px;color:'+(q?"#F87171":"rgba(255,255,255,0.6)")+';font-weight:600;">('+U+")</span>"+(M.dayOhaeng?'<span style="font-size:10px;color:'+le.c+';font-weight:700;">'+Array.from(M.dayOhaeng).join("·")+"일</span>":"")+"</div>"}).join("");i+='<div class="ds-card" style="background:rgba('+s.r+",0.06);border:1px solid rgba("+s.r+",0.22);border-left:4px solid "+s.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+s.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+s.r+',0.2);">📅 추천 방문 날짜'+(f?"":'<span style="font-size:11px;font-weight:400;opacity:.45;margin-left:8px;">상위 3일 · 전체는 멤버 전용</span>')+"</div>",i+='<div style="display:flex;flex-wrap:wrap;gap:2px;">'+x+"</div>",!f&&u.length>=3&&(i+='<div style="margin-top:12px;font-size:12px;color:rgba(255,255,255,0.35);text-align:center;">🔒 멤버십 회원은 최대 15일 추천 날짜 제공</div>'),i+="</div>"}if(Object.keys(_).length>0){const s=j.dist,d=Object.entries(_).map(function([x,M]){const v=x===$.templeOhaeng,g=t[x]||t.금;return'<div style="display:inline-flex;align-items:center;gap:6px;padding:8px 18px;border-radius:24px;margin:4px;font-weight:800;font-size:14px;background:rgba('+g.r+","+(v?"0.18":"0.07")+");border:1.5px solid rgba("+g.r+","+(v?"0.55":"0.2")+");color:"+(v?g.c:"rgba(255,255,255,0.55)")+";"+(v?"box-shadow:0 0 14px rgba("+g.r+",0.25);":"")+'">'+(W[x]||"")+" "+x+" "+M+(v?" ✦":"")+"</div>"}).join("");i+='<div class="ds-card" style="background:rgba('+s.r+",0.06);border:1px solid rgba("+s.r+",0.22);border-left:4px solid "+s.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+s.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+s.r+',0.2);">🔥 나의 사주 오행 분포</div>',i+='<div style="display:flex;flex-wrap:wrap;gap:2px;margin-bottom:10px;">'+d+"</div>";var y=a?a.weak&&a.weak.부족오행:null,ne=y&&y===$.templeOhaeng?'✦ 내 사주에서 가장 부족한 <b style="color:'+w+';">'+$.templeOhaeng+"</b> 기운을 이 사찰이 채워줍니다.":'✦ 이 사찰의 기운은 <b style="color:'+w+';">'+($.templeOhaeng||"")+"</b>으로, 선택하신 기도목적("+(L||"—")+")에 최적화된 도량입니다.";i+='<div style="font-size:12px;color:rgba(255,255,255,0.4);line-height:1.7;margin-top:6px;">'+ne+"</div>",i+="</div>"}f||(i+='<div style="background:rgba(212,175,55,0.06);border:1px solid rgba(212,175,55,0.3);border-radius:16px;padding:28px 20px;margin-bottom:12px;text-align:center;">',i+='<div style="font-size:30px;margin-bottom:10px;">🔒</div>',i+='<div style="font-size:16px;font-weight:800;color:#D4AF37;margin-bottom:8px;">멤버십 전용 콘텐츠</div>',i+='<div style="font-size:13px;color:rgba(255,255,255,0.5);margin-bottom:18px;">유래 전문 · 추천 날짜 전체 · 기도문 전체 &amp; 복사</div>',i+='<div class="member-unlock"><input type="text" id="detail-member-code-input" placeholder="멤버십 코드 입력" /><button id="detail-member-code-btn">확인</button></div>',i+="</div>"),i+='<div class="patent-notice-banner" style="margin-bottom:12px;"><div class="patent-notice-icon">⚖️</div><div class="patent-notice-body"><div class="patent-notice-title">지식재산권 안내</div><div class="patent-notice-text">본 서비스의 <strong>인연 시너지 산출 로직</strong>은 비가산 시너지 기반 지수 산출 방식을 적용한 독자 기술입니다.</div><span class="patent-num">특허 출원 중 (출원번호: 40-2026-00*****)</span></div></div>',i+="</div>",E.innerHTML=i,function(){const d=m.name;if(!d)return;const x=document.getElementById("detail-hero-img"),M=document.getElementById("detail-hero-overlay"),v=document.getElementById("detail-hero");if(!x||!M)return;function g(P){if(!P)return!1;var N=P.toLowerCase();if(!/\.(jpe?g)/i.test(N))return!1;var ie=["flag","taegeuk","taeguk","portrait","map","document","signature","seal","coin","stamp","symbol","logo","icon","graph","chart","diagram","hangeul","hangul","korean_language","korea_map","location","emblem","crest","arms","건물","도로","지도","교통","버스","지하철","아파트"];return!ie.some(function(de){return N.indexOf(de)!==-1})}function Y(P){var N=new Image;N.onload=function(){N.naturalWidth<300||N.naturalHeight<200||(x.style.backgroundImage="url("+P+")",x.style.opacity="1",M.style.opacity="1",v&&(v.style.background="none"),x.style.cursor="zoom-in",x.onclick=function(){var ie=document.createElement("div");ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:99999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;";var de=new Image;de.src=P,de.style.cssText="max-width:92vw;max-height:88vh;border-radius:12px;box-shadow:0 8px 48px #000;",ie.appendChild(de),ie.onclick=function(){document.body.removeChild(ie)},document.body.appendChild(ie)})},N.src=P}var X={목:"linear-gradient(160deg,rgba(34,85,34,0.85) 0%,rgba(20,60,20,0.95) 100%)",화:"linear-gradient(160deg,rgba(120,40,10,0.85) 0%,rgba(80,20,5,0.95) 100%)",토:"linear-gradient(160deg,rgba(90,70,20,0.85) 0%,rgba(60,45,10,0.95) 100%)",금:"linear-gradient(160deg,rgba(90,70,10,0.85) 0%,rgba(60,45,5,0.95) 100%)",수:"linear-gradient(160deg,rgba(10,40,80,0.85) 0%,rgba(5,20,60,0.95) 100%)"};function U(){var P=$.templeOhaeng||"금",N=X[P]||X.금;v&&(v.style.background=N)}function q(P){return fetch("https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch="+encodeURIComponent(P)+"&srnamespace=6&srlimit=10&srprop=&format=json&origin=*").then(function(N){return N.ok?N.json():null}).then(function(N){if(!N||!N.query||!N.query.search)return[];var ie=N.query.search.map(function(ve){return ve.title}).filter(function(ve){return/\.jpe?g$/i.test(ve)}).slice(0,8);if(!ie.length)return[];var de=ie.map(encodeURIComponent).join("%7C");return fetch("https://commons.wikimedia.org/w/api.php?action=query&prop=imageinfo&iiprop=url%7Csize&iiurlwidth=900&titles="+de+"&format=json&origin=*").then(function(ve){return ve.ok?ve.json():null}).then(function(ve){if(!ve)return[];var je=ve.query&&ve.query.pages?ve.query.pages:{};return Object.values(je).map(function(he){return he.imageinfo&&he.imageinfo[0]}).filter(function(he){return he&&he.thumburl&&g(he.thumburl)&&(he.width||0)>=400&&(he.height||0)>=300})})}).catch(function(){return[]})}var H=!1;function le(P){return H||!P||!P.length?!1:(H=!0,Y(P[0].thumburl),!0)}fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=pageimages|info&pithumbsize=900&titles="+encodeURIComponent(d)+"&format=json&origin=*").then(function(P){return P.ok?P.json():null}).then(function(P){if(!H&&P){var N=P.query&&P.query.pages?P.query.pages:{},ie=Object.values(N)[0];if(!(!ie||ie.missing!==void 0)){var de=ie.thumbnail?ie.thumbnail.source:null;de&&g(de)&&(H=!0,Y(de))}}}).catch(function(){}),setTimeout(function(){H||Promise.all([q(d+" 사찰"),q(d+" temple Korea")]).then(function(P){if(!H){var N=[].concat(P[0],P[1]);if(!le(N)){var ie=(m.address||"").split(" ").slice(0,2).join(" ");return ie?q(ie+" 불교 사찰"):Promise.resolve([])}}}).then(function(P){H||!P||le(P)||U()}).catch(function(){H||U()})},800)}(),function(){var d=document.getElementById("temple-gallery"),x=document.getElementById("gallery-scroll");if(!d||!x||!m.name)return;function M(g){if(!(!g||g.length===0)){var Y=g.map(function(X){var U=X.thumburl,q=U.replace(/\/\d+px-/,"/1200px-");return`<div style="flex:0 0 auto;width:160px;height:110px;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,0.12);cursor:pointer;transition:transform .2s,box-shadow .2s;" onmouseover="this.style.transform='scale(1.04)';this.style.boxShadow='0 6px 20px rgba(0,0,0,.5)'" onmouseout="this.style.transform='';this.style.boxShadow=''" data-src="`+q+`" onclick="(function(u){var ov=document.createElement('div');ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.9);z-index:99999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;';var im=new Image();im.src=u;im.style.cssText='max-width:92vw;max-height:88vh;border-radius:10px;box-shadow:0 8px 40px #000;';ov.appendChild(im);ov.onclick=function(){document.body.removeChild(ov);};document.body.appendChild(ov);})(this.dataset.src)"><img src="`+U+`" style="width:100%;height:100%;object-fit:cover;" loading="lazy" onerror="this.parentElement.style.display='none'"></div>`}).join("");x.innerHTML=Y,d.style.display="block"}}function v(g,Y){var X=Y.map(function(U){return encodeURIComponent(U)}).join("%7C");return fetch(g+"?action=query&prop=imageinfo&iiprop=url%7Csize&iiurlwidth=500&titles="+X+"&format=json&origin=*").then(function(U){return U.ok?U.json():null}).then(function(U){if(!U)return[];var q=U.query&&U.query.pages?U.query.pages:{};return Object.values(q).map(function(H){return H.imageinfo&&H.imageinfo[0]}).filter(function(H){if(!H||!H.thumburl||!/\.(jpe?g)/i.test(H.thumburl)||(H.width||0)<400||(H.height||0)<300)return!1;var le=H.thumburl.toLowerCase(),P=["taegeuk","taeguk","flag","portrait","map","document","signature","seal","coin","stamp","symbol","logo","icon","graph","chart","diagram","hangeul","hangul","emblem","crest","location","korea_map","korean_language","태극","지도","문서","초상","건물","도로","교통"];if(P.some(function(ie){return le.indexOf(ie)!==-1}))return!1;var N=(H.width||1)/(H.height||1);return!(N<.5||N>3.5)})})}fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=pageprops&titles="+encodeURIComponent(m.name)+"&format=json&origin=*").then(function(g){return g.ok?g.json():null}).then(function(g){var Y=g&&g.query&&g.query.pages?g.query.pages:{},X=Object.values(Y)[0],U=X&&X.pageprops&&X.pageprops.commonsCategory;return U?fetch("https://commons.wikimedia.org/w/api.php?action=query&list=categorymembers&cmtitle="+encodeURIComponent("Category:"+U)+"&cmtype=file&cmlimit=20&format=json&origin=*").then(function(q){return q.ok?q.json():null}).then(function(q){if(!q||!q.query)return null;var H=(q.query.categorymembers||[]).map(function(le){return le.title}).filter(function(le){return/\.jpe?g$/i.test(le)}).slice(0,10);return H.length?v("https://commons.wikimedia.org/w/api.php",H):null}):null}).then(function(g){return g&&g.length>=1?(M(g),Promise.resolve(!0)):fetch("https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch="+encodeURIComponent(m.name+" temple")+"&srnamespace=6&srlimit=16&srprop=&format=json&origin=*").then(function(Y){return Y.ok?Y.json():null}).then(function(Y){if(!Y||!Y.query||!Y.query.search)return null;var X=Y.query.search.map(function(U){return U.title}).filter(function(U){return/\.jpe?g$/i.test(U)}).slice(0,10);return X.length?v("https://commons.wikimedia.org/w/api.php",X):null})}).then(function(g){if(g===!0||g&&g.length>=1){g!==!0&&M(g);return}return fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=images&titles="+encodeURIComponent(m.name)+"&imlimit=20&format=json&origin=*").then(function(Y){return Y.ok?Y.json():null}).then(function(Y){if(Y){var X=Y.query&&Y.query.pages?Y.query.pages:{},U=Object.values(X)[0];if(!(!U||!U.images)){var q=U.images.map(function(H){return H.title}).filter(function(H){return/\.jpe?g$/i.test(H)}).slice(0,10);if(q.length)return v("https://ko.wikipedia.org/w/api.php",q).then(function(H){H&&H.length>=1&&M(H)})}}})}).catch(function(){})}(),function(){if(m.name){var d=document.getElementById("history-section");document.getElementById("history-body"),!(T.length>=150)&&fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=1&explaintext=1&exsentences=6&titles="+encodeURIComponent(m.name)+"&format=json&origin=*").then(function(x){return x.ok?x.json():null}).then(function(x){if(x){var M=x.query&&x.query.pages?x.query.pages:{},v=Object.values(M)[0],g=v&&v.extract?v.extract.trim():"";if(!(!g||g.length<30)&&d){var Y=(T?T+`

`:"")+g,X=f?Y:Y.slice(0,200)+(Y.length>200?'… <span style="color:rgba(255,255,255,0.35);font-size:12px;">🔒 전체보기는 멤버 전용</span>':"");d.innerHTML='<div class="ds-card" style="background:rgba(217,119,6,0.06);border:1px solid rgba(217,119,6,0.22);border-left:4px solid #D97706;border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;"><div style="font-size:14px;font-weight:800;color:#D97706;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(217,119,6,0.2);">📜 유래 · 연혁 <span style="font-size:11px;font-weight:400;opacity:.45;margin-left:6px;">위키백과 참조</span></div><div style="font-size:14px;color:#E2E8F0;line-height:1.9;white-space:pre-line;">'+X+"</div></div>"}}}).catch(function(){})}}();const pe=document.getElementById("gido-copy-btn"),ge=document.getElementById("gido-raw");pe&&ge&&pe.addEventListener("click",function(){navigator.clipboard.writeText(ge.value).then(function(){pe.textContent="✓ 복사됨!",setTimeout(function(){pe.innerHTML="📋 기도문 복사"},2e3)}).catch(function(){ge.select(),document.execCommand("copy")})}),(me=document.getElementById("detail-back-btn"))==null||me.addEventListener("click",()=>{var s;typeof A=="function"?A():(E.innerHTML="",E.classList.add("hidden"),p&&(p.style.display=""),(s=document.getElementById("app"))==null||s.scrollIntoView({behavior:"smooth",block:"start"}))});const ue=document.getElementById("detail-member-code-btn"),se=document.getElementById("detail-member-code-input");ue&&ue.addEventListener("click",()=>{se&&se.value.trim()===Ge?(Re(se.value.trim()),Le(e,a,!0,A)):alert("코드가 올바르지 않습니다.")})}function Pe(e){var $,n;const a=document.getElementById("results");a.classList.remove("hidden");const f=(e.results||[])[0],A=it[($=f==null?void 0:f.detail)==null?void 0:$.bearing]??0,E=_e();a.innerHTML=`
  ...
  `;const p=document.getElementById("member-code-input"),m=document.getElementById("member-code-btn");m&&m.addEventListener("click",()=>{p&&p.value.trim()===Ge?(Re(p.value.trim()),Pe(e)):alert("코드가 올바르지 않습니다.")}),document.querySelectorAll(".detail-view-btn").forEach(t=>{t.addEventListener("click",()=>{const z=parseInt(t.dataset.templeIndex);Le(e.results[z],e,E)})}),function(){var b,k,_,L;var z=(b=e.results)==null?void 0:b[0],w=((k=z==null?void 0:z.temple)==null?void 0:k.name)||"인연사찰",l=(z==null?void 0:z.score)||"",j=e.purpose||"",S="https://jamgong-inyeonsachal.vercel.app",W='🏯 나의 인연사찰은 "'+w+`"!
오행 궁합 `+l+"점 매칭 ("+j+`)
...
`+W)})}),(L=document.getElementById("share-copy-btn"))==null||L.addEventListener("click",function(){var u;var C=document.getElementById("share-copy-btn");(u=navigator.clipboard)==null||u.writeText(S).then(function(){C&&(C.textContent="✅ 복사됨!",setTimeout(function(){C.innerHTML="🔗 링크 복사"},2e3))}).catch(function(){alert("주소: "+S)})})}(),(n=document.getElementById("demo-detail-btn"))==null||n.addEventListener("click",()=>{Le({score:94,reason:"수(水) 기운이 부족한 사주에 이 사찰의 강한 수 기운이 보완해줍니다.",temple:{name:"통도사",address:"경상남도 양산시 하북면 통도사로 108",lat:35.489166,lng:129.058611,foundedYear:646},detail:{templeOhaeng:"수",bearing:"북",distanceKm:12.3},weather:{condition:"맑음",temp:24}},{purpose:"학업운",distribution:{목:1,화:2,토:2,금:2,수:1},targetOhaeng:"수",purposeGuide:[]},!0)}),a.scrollIntoView({behavior:"smooth"})}function at(e,a){const f={목:"화",화:"토",토:"금",금:"수",수:"목"},A={목:"토",토:"수",수:"화",화:"금",금:"목"},E={목:"木",화:"火",토:"土",금:"金",수:"水"},p={목:"#64DCA0",화:"#FF6B9D",토:"#FB923C",금:"#FFB347",수:"#00D2FF"};let m,$,n,t,z,w;const l=f[e]===a,j=f[a]===e,S=A[e]===a,W=A[a]===e;return l||j?(m=88,$="상생(相生)",n="천생연분(天生緣分)",t="#00D2FF",z="0,210,255",w="오행이 서로를 생(生)해주는 가장 이상적인 조합입니다. 단, 한쪽이 지나치게 의존하면 소진될 수 있습니다."):e===a?(m=68,$="비화(比和)",n="동질형(同質型)",t="#9FE5C4",z="159,229,196",w="같은 오행으로 서로를 잘 이해하지만, 동일한 약점을 함께 지닙니다. 외부 자극 없이는 정체될 수 있습니다."):S||W?(m=42,$="상극(相剋)",n="갈등형(葛藤型)",t="#F5A623",z="245,166,35",w="오행이 서로를 극(克)합니다. 긴장과 갈등이 잦을 수 있으며, 의식적인 노력 없이는 관계가 소모적이 될 수 있습니다."):(m=62,$="중화(中和)",n="평범형(平凡型)",t="#FF6B9D",z="255,107,157",w="특별히 맞거나 부딪히는 기운이 없는 평이한 조합입니다. 큰 시너지보다는 무난한 관계가 됩니다."),{score:m,relation:$,grade:n,color:t,rgb:z,comment:w,symA:E[e]||e,symB:E[a]||a,colA:p[e]||"#fff",colB:p[a]||"#fff"}}function lt(e){var K,be,Fe,Ce,o,ye,ze,Se,Me,Oe,Ie,De;const a=document.getElementById("results");a.classList.remove("hidden");const f=_e(),A=((Fe=(be=(K=e.pillarsA)==null?void 0:K[2])==null?void 0:be.wx)==null?void 0:Fe[0])||e.targetA,E=((ye=(o=(Ce=e.pillarsB)==null?void 0:Ce[2])==null?void 0:o.wx)==null?void 0:ye[0])||e.targetB,p=at(A,E),m=(e.hapChung||[]).filter(r=>r.positive).length,$=(e.hapChung||[]).filter(r=>!r.positive).length,n=m*4-$*7,t=Math.max(10,Math.min(99,p.score+n)),z=Math.round(t);window._sajuContext=null,window._gunghamContext={pillarsA:e.pillarsA,pillarsB:e.pillarsB,hapChung:e.hapChung,distributionA:e.distributionA,distributionB:e.distributionB,genderA:e.genderA,genderB:e.genderB,targetA:A,targetB:E,finalScore:t,relation:p.relation,grade:p.grade};const w={목:"#64DCA0",화:"#FF6B9D",토:"#FB923C",금:"#FFB347",수:"#00D2FF"},l={목:"木",화:"火",토:"土",금:"金",수:"水"};function j(r,O,R){return!r||r.length===0?"":`
    <div class="gh-pillar-col">
      ...
      ${R}
    </div>`},re=r=>{var J;const O=[r.a,r.b].sort().join(""),R=oe[O]||oe[[r.b,r.a].join("")]||"";return`<div class="gh-hap-item negative">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        <span class="gh-hap-type">${r.type}</span>
        <span class="gh-hap-chars">${r.a} ↔ ${r.b}</span>
        ${(J=r.pillars)!=null&&J.length?`<span class="gh-hap-pill">${r.pillars.join(" / ")}</span>`:""}
    ...
    </div>`:"",G=e.distributionA||{},c=e.distributionB||{},h=["목","화","토","금","수"],F={목:"木(목)",화:"火(화)",토:"土(토)",금:"金(금)",수:"水(수)"},B={목:"창의·성장·인자함",화:"열정·표현·감수성",토:"안정·신뢰·중재력",금:"결단·원칙·날카로움",수:"지혜·직관·유연성"},y={};I.forEach(r=>{const O=[r.a,r.b].sort().join("");if((r.type||"").includes("천간합")){const R=S[O]||S[[r.b,r.a].join("")];R!=null&&R.wx&&(y[R.wx]=(y[R.wx]||0)+1)}});const ne={목:"창의·성장",화:"정(情)·열정·인연",토:"안정·신뢰",금:"의리·결속",수:"지혜·유연성"},pe=Object.entries(y).sort((r,O)=>O[1]-r[1]),ge=pe.length>0?"✨ 합화오행: "+pe.map(([r,O])=>`<strong style="color:${w[r]||"#fff"}">${F[r]||r}(${ne[r]})</strong>${O>1?` ×${O}`:""}`).join(" · ")+" 기운이 두 분 사이에서 생성됩니다.":"",ue=te?`
    <div class="gh-card">
      <div class="gh-section">
    ...
    </div>`:"",se={};h.forEach(r=>{se[r]=(G[r]||0)+(c[r]||0)});const me=[...h].sort((r,O)=>(se[r]||0)-(se[O]||0)),s=me[0],d=me[me.length-1],x=h.filter(r=>Math.abs((G[r]||0)-(c[r]||0))>=2);let M="";if(s&&se[s]<=1&&(M+=`<div class="gh-dist-interp">⚠ 두 분 모두 <strong style="color:${w[s]}">${F[s]}</strong>(${B[s]})이 부족합니다. 이 기운이 약한 영역에서 함께 어려움을 겪을 수 있습니다.</div>`),x.length>0){const r=x[0],O=(G[r]||0)>(c[r]||0);M+=`<div class="gh-dist-interp good">✓ <strong style="color:${w[r]}">${F[r]}</strong>에서 한 분이 강하고 다른 분이 약합니다. ${O?"나":"상대방"}의 ${F[r]} 기운이 ${O?"상대방":"나"}를 채워주는 긍정적인 보완 관계입니다.</div>`}se[d]>=6&&(M+=`<div class="gh-dist-interp warn">△ 두 분 합산 <strong style="color:${w[d]}">${F[d]}</strong>이 과도합니다. 이 오행의 특성(${B[d]})이 지나치게 강해 균형을 잃을 수 있습니다.</div>`);const v=new Set(["甲","丙","戊","庚","壬"]),g=new Set(["乙","丁","己","辛","癸"]),Y=new Set(["子","寅","辰","午","申","戌"]),X=new Set(["丑","卯","巳","未","酉","亥"]),U=r=>{let O=0,R=0,J=0,Q=0;return r.forEach(ae=>{v.has(ae.stem)?O++:g.has(ae.stem)&&R++,Y.has(ae.branch)?J++:X.has(ae.branch)&&Q++}),{yangStem:O,yinStem:R,yangBranch:J,yinBranch:Q}},q=U(e.pillarsA||[]),H=U(e.pillarsB||[]),le=q.yangStem+q.yangBranch,P=q.yinStem+q.yinBranch,N=H.yangStem+H.yangBranch,ie=H.yinStem+H.yinBranch,de=(()=>{const r=le>P?"양(陽)형":le<P?"음(陰)형":"균형형",O=N>ie?"양(陽)형":N<ie?"음(陰)형":"균형형",R={"양(陽)형":"활동적이고 외향적입니다. 먼저 나서고 에너지를 발산하는 편입니다.","음(陰)형":"내성적이고 수용적입니다. 관찰하고 흡수하며 내면이 깊습니다.",균형형:"음양이 고르게 분포되어 상황에 따라 유연하게 대처합니다."};let J="";return r!==O?J="서로 다른 음양 기질이 만나 자연스러운 끌림이 생깁니다. 보완 관계가 형성됩니다.":r==="양(陽)형"?J="두 분 모두 적극적이라 활기차지만 주도권 갈등이 생길 수 있습니다.":r==="음(陰)형"?J="두 분 모두 내성적이라 서로를 깊이 이해하지만 먼저 나서는 것을 꺼릴 수 있습니다.":J="두 분 모두 균형 잡힌 기질로 상황 적응력이 좋습니다.",{typeA:r,typeB:O,descA:R[r],descB:R[O],chem:J}})(),ve=`
    <div class="gh-section">
      <div class="gh-section-title">🌊 오행 분포 비교</div>
      <div class="gh-dist-grid">
        ${h.map(r=>{const O=G[r]||0,R=c[r]||0,J=w[r]||"#fff";return`
          <div class="gh-dist-row">
            <span class="gh-dist-name" style="color:${J}">${l[r]||r}(${r})</span>
            <div class="gh-dist-bars">
              <div class="gh-bar-wrap">
                <div class="gh-bar gh-bar-a" style="width:${O/8*100}%;background:${J}"></div>
              </div>
    ...
    <div style="margin-bottom:10px;padding:10px 13px;background:rgba(255,255,255,0.04);border-radius:10px;border:1px solid rgba(255,255,255,0.08);font-size:12.5px;line-height:1.7;color:rgba(255,255,255,0.7);">
      ...
      <div class="gh-advice-group">
        ...
        ${fe.strength.map(r=>`<div class="gh-advice-item positive">✓ ${r}</div>`).join("")}
      </div>
      <div class="gh-advice-group" style="margin-top:10px">
        <div class="gh-advice-label caution">주의 요소</div>
        ${fe.caution.map(r=>`<div class="gh-advice-item caution">⚠ ${r}</div>`).join("")}
      </div>
    </div>`;a.innerHTML=`
    <!-- 홈 버튼 -->
    <div class="detail-nav-row" style="margin-bottom:16px;">
    ...
    <div class="gh-card">
      <div class="gh-section">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;margin-top:8px;">
          ...
          ${J&&Q&&O===R?'<div style="margin-top:4px;padding:8px 12px;background:rgba(255,215,0,0.07);border-radius:8px;font-size:12px;color:rgba(255,215,0,0.8);">✨ 두 분이 같은 일주(同日柱)입니다. 매우 드문 인연으로, 서로를 거울처럼 반추하는 관계입니다.</div>':""}
        </div>
      </div>
    </div>`})():""}
