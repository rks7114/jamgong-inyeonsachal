  ...
  `;let e=Ke[0];document.querySelectorAll(".purpose-chip").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll(".purpose-chip").forEach(t=>t.classList.remove("active")),n.classList.add("active"),e=n.dataset.purpose})});let a="solar";document.querySelectorAll("[data-calendar]").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("[data-calendar]").forEach(t=>t.classList.remove("active")),n.classList.add("active"),a=n.dataset.calendar,document.getElementById("leap-month-wrap").classList.toggle("hidden",a!=="lunar")})}),(m=document.getElementById("birth-hour"))==null||m.addEventListener("change",n=>{const t=document.getElementById("birth-minute");t&&(t.disabled=n.target.value==="")}),($=document.getElementById("birth-city"))==null||$.addEventListener("change",n=>{const t=document.getElementById("birth-lng-display");t&&(t.textContent=n.target.value?"경도 "+n.target.value+"°":"")});let f="male";document.querySelectorAll(".gender-btn").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll(".gender-btn").forEach(t=>t.classList.remove("active")),n.classList.add("active"),f=n.dataset.gender})});let A="solar";document.querySelectorAll("[data-calendar-b]").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("[data-calendar-b]").forEach(t=>t.classList.remove("active")),n.classList.add("active"),A=n.dataset.calendarB})});let E="female";document.querySelectorAll(".gender-btn-b").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll(".gender-btn-b").forEach(t=>t.classList.remove("active")),n.classList.add("active"),E=n.dataset.genderB})});let p="solo";document.querySelectorAll(".mode-toggle-btn").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll(".mode-toggle-btn").forEach(S=>S.classList.remove("active")),n.classList.add("active"),p=n.dataset.mode;const t=document.getElementById("birth-b-field"),z=document.getElementById("birth-label-a"),w=document.getElementById("submit-btn"),l=document.getElementById("purpose-field"),j=document.getElementById("location-field");if(p==="couple")t.classList.remove("hidden"),t.style.display="flex",z.innerHTML='<span class="label-badge-a">나</span> 생년월일시',w.textContent="함께 인연사찰 찾기",l&&(l.style.display=""),j&&(j.style.display="");else if(p==="saju"){t.classList.add("hidden"),t.style.display="none",z.innerHTML='생년월일시 <span class="help-tip" tabindex="0">?<span class="help-tip-bubble">사주 팔자 계산의 기준이 되는 정보입니다.</span></span>',w.textContent="🔮 사주 팔자 확인",l&&(l.style.display="none"),j&&(j.style.display="none"),document.getElementById("match-form").style.display="",document.getElementById("temple-search-wrap").style.display="";const S=document.getElementById("dream-page");S&&(S.style.display="none")}else if(p==="dream"){document.getElementById("match-form").style.display="none",document.getElementById("temple-search-wrap").style.display="none";let S=document.getElementById("dream-page");S||(dt(),S=document.getElementById("dream-page")),S&&(S.style.display="")}else{t.classList.add("hidden"),t.style.display="none",z.innerHTML='생년월일시 <span class="help-tip" tabindex="0">?<span class="help-tip-bubble">사주 오행 계산의 기준이 되는 정보입니다. 시간을 모르셔도 괜찮습니다 — "시간 모름"을 선택하시면 정오 기준으로 계산됩니다.</span></span>',w.textContent="인연사찰 찾기",l&&(l.style.display=""),j&&(j.style.display=""),document.getElementById("match-form").style.display="",document.getElementById("temple-search-wrap").style.display="";const S=document.getElementById("dream-page");S&&(S.style.display="none")}})}),document.getElementById("match-form").addEventListener("submit",async n=>{var _,F,C,u,V,T,te,Z;n.preventDefault();const t=document.getElementById("birth-year").value,z=document.getElementById("birth-month").value,w=document.getElementById("birth-day").value,l=document.getElementById("birth-hour").value,j=parseInt((_=document.getElementById("birth-minute"))==null?void 0:_.value)||0,S=Math.min(59,Math.max(0,j)),W=((F=document.getElementById("birth-city"))==null?void 0:F.value)||"",oe=((C=document.getElementById("is-leap-month"))==null?void 0:C.checked)||!1;if(!t||!z||!w){alert("생년월일(연도·월·일)을 모두 선택해주세요.");return}const b={calendarType:a,year:parseInt(t),month:parseInt(z),day:parseInt(w),hour:l!==""?parseInt(l):12,minute:S,isLeapMonth:oe,birthLongitude:W!==""?parseFloat(W):null,gender:f};if(p==="couple"){const H=document.getElementById("birth-year-b").value,I=document.getElementById("birth-month-b").value,i=document.getElementById("birth-day-b").value;if(!H||!I||!i){alert("상대방 생년월일(연도·월·일)을 모두 선택해주세요.");return}}if(p==="saju"){const H=document.getElementById("submit-btn");H.disabled=!0;const I=document.getElementById("match-form");I&&(I.style.display="none");const i=document.getElementById("results");i.classList.remove("hidden");const ee=[{icon:"🔍",text:"위치 감지 중..."},{icon:"🌀",text:"사주 오행 분석 중..."},{icon:"✨",text:"AI 풀이 생성 중..."},{icon:"🏯",text:"인연사찰 탐색 중..."},{icon:"🔮",text:"팔자 기운 살피는 중..."}];let re=0;(()=>{const c=ee[re%ee.length];i.innerHTML=`
          <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:320px;gap:28px;padding:40px 20px;">
            ...
            <div style="display:flex;gap:6px;">
              ${ee.map((h,L)=>`<div class="saju-dot" style="width:6px;height:6px;border-radius:50%;background:${L===re%ee.length?"rgba(0,210,255,0.9)":"rgba(255,255,255,0.15)"};transition:background .3s;"></div>`).join("")}
            </div>
          </div>`})();const G=setInterval(()=>{re++;const c=ee[re%ee.length],h=document.getElementById("saju-loading-text");h&&(h.textContent=c.text),document.querySelectorAll(".saju-dot").forEach((B,y)=>{B.style.background=y===re%ee.length?"rgba(0,210,255,0.9)":"rgba(255,255,255,0.15)"});const L=i.querySelector("[style*='font-size:28px']");L&&(L.textContent=c.icon)},2200);try{const[c,h]=await Promise.all([fetch("/api/saju",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({birthInput:b})}),Ne()]),L=await c.json();if(L.error){clearInterval(G),i.classList.add("hidden"),I&&(I.style.display=""),alert(L.error);return}let B=null,y=null;const ne=h.userLat??37.5665,pe=h.userLng??126.978,ge=ue=>new Promise((se,me)=>setTimeout(()=>me(new Error("timeout")),ue));try{const[ue,se]=await Promise.all([fetch("/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({birthInput:b,userLat:ne,userLng:pe,purpose:"인연운"})}),Promise.race([fetch("/api/saju-explain",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({eightChar:L.eightChar,distribution:L.distribution,weak:L.weak,daYun:L.daYun?{...L.daYun,list:(u=L.daYun.list)==null?void 0:u.filter(me=>me.isCurrent||me.startAge>=30)}:null,samjae:L.samjae,birthInput:b})}),ge(38e3)]).catch(()=>null)]);ue!=null&&ue.ok&&(B=await ue.json()),se!=null&&se.ok&&(y=await se.json())}catch(ue){console.warn("병렬 오류:",ue)}clearInterval(G),We(L,b,B,(y==null?void 0:y.explanation)||null)}catch{clearInterval(G),i.classList.add("hidden"),i.innerHTML="",I&&(I.style.display=""),alert("사주 계산 중 오류가 발생했습니다.")}finally{H.disabled=!1,H.textContent="🔮 사주 팔자 확인"}return}const k=document.getElementById("submit-btn");if(k.disabled=!0,p==="couple"){const H=document.getElementById("match-form"),I=document.getElementById("results");H&&(H.style.display="none"),I.classList.remove("hidden");const i=[{icon:"💑",text:"두 분의 기운을 살피는 중..."},{icon:"🔮",text:"팔자 여덟 글자 분석 중..."},{icon:"⚡",text:"합충(合沖) 관계 탐색 중..."},{icon:"🌊",text:"오행 시너지 계산 중..."},{icon:"🏯",text:"두 분의 인연 기운 정리 중..."},{icon:"✨",text:"궁합 결과 생성 완료 중..."}];let ee=0;(()=>{const G=i[ee%i.length];I.innerHTML=`
          ...
          </div>`})();const ve=setInterval(()=>{ee++;const G=i[ee%i.length],c=document.getElementById("couple-loading-text");c&&(c.textContent=G.text);const h=I.querySelector("[style*='font-size:30px']");h&&(h.textContent=G.icon),I.querySelectorAll(".couple-dot").forEach((L,B)=>{L.style.background=B===ee%i.length?"rgba(245,100,200,0.9)":"rgba(255,255,255,0.15)"})},5e3);try{const G=document.getElementById("birth-year-b").value,c=document.getElementById("birth-month-b").value,h=document.getElementById("birth-day-b").value,L=document.getElementById("birth-hour-b").value,B={calendarType:A,year:parseInt(G),month:parseInt(c),day:parseInt(h),hour:L!==""?parseInt(L):12,minute:0,isLeapMonth:!1,gender:E},y=await Ne().catch(()=>({})),ne=y.userLat??37.5665,pe=y.userLng??126.978,ge=new Promise(me=>setTimeout(me,5e3)),[ue]=await Promise.all([fetch("/api/match-couple",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({birthInputA:b,birthInputB:B,purpose:e,userLat:ne,userLng:pe,memberUnlocked:_e(),region:((V=document.getElementById("region-select"))==null?void 0:V.value)||"",maxDistanceKm:parseInt((T=document.getElementById("distance-select"))==null?void 0:T.value)||null})}),ge]),se=await ue.json();if(clearInterval(ve),se.error){I.classList.add("hidden"),I.innerHTML="",H&&(H.style.display=""),alert(`오류: ${se.error}
생년월일을 다시 확인해주세요.`);return}se.purpose=e,lt(se)}catch{clearInterval(ve),I.classList.add("hidden"),I.innerHTML="",H&&(H.style.display=""),alert("궁합 분석 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.")}finally{k.disabled=!1,k.textContent="💑 궁합"}return}{const H=document.getElementById("match-form"),I=document.getElementById("results");H&&(H.style.display="none"),I.classList.remove("hidden");const i=[{icon:"🔍",text:"위치 감지 중..."},{icon:"🌀",text:"오행 기운 분석 중..."},{icon:"🏯",text:"인연사찰 탐색 중..."},{icon:"✨",text:"나의 인연을 살피는 중..."},{icon:"🔮",text:"사주 기운 정렬 중..."},{icon:"🌸",text:"결과 생성 완료 중..."}];let ee=0;(()=>{const G=i[ee%i.length];I.innerHTML=`
          <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:340px;gap:24px;padding:40px 20px;">
            <div style="position:relative;width:88px;height:88px;">
              <svg viewBox="0 0 88 88" style="width:88px;height:88px;animation:spin 2s linear infinite;">
          ...
          </div>`})();const ve=setInterval(()=>{ee++;const G=i[ee%i.length],c=document.getElementById("solo-loading-text");c&&(c.textContent=G.text);const h=I.querySelector("[style*='font-size:30px']");h&&(h.textContent=G.icon),I.querySelectorAll(".solo-dot").forEach((L,B)=>{L.style.background=B===ee%i.length?"rgba(0,210,255,0.9)":"rgba(255,255,255,0.15)"})},5e3);try{const G=new Promise(ne=>setTimeout(ne,5e3)),c=await Ne().catch(()=>({})),h=c.userLat??37.5665,L=c.userLng??126.978,[B]=await Promise.all([fetch("/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({birthInput:b,purpose:e,userLat:h,userLng:L,memberUnlocked:_e(),region:((te=document.getElementById("region-select"))==null?void 0:te.value)||"",maxDistanceKm:parseInt((Z=document.getElementById("distance-select"))==null?void 0:Z.value)||null})}),G]);if(!B.ok){const ne=await B.text().catch(()=>"");throw new Error(`HTTP ${B.status}: ${ne.slice(0,300)}`)}const y=await B.json();if(clearInterval(ve),y.error){I.classList.add("hidden"),I.innerHTML="",H&&(H.style.display=""),alert(`오류: ${y.error}
생년월일을 다시 확인해주세요.`);return}y.purpose=e,Pe(y)}catch(G){clearInterval(ve),I.classList.add("hidden"),I.innerHTML="",H&&(H.style.display=""),alert(`매칭 오류: ${G.message}`)}finally{k.disabled=!1,k.textContent="인연사찰 찾기"}}})}function rt(e,a){const f=e.eightChar;if(!f)return"";const A=o=>(o==null?void 0:o[0])||"",E=o=>(o==null?void 0:o[1])||"",p=A(f.day),m=A(f.month),$=A(f.year),n=A(f.time),t=E(f.day),z=E(f.month),w=E(f.year),l=E(f.time),j=[l,t,z,w].filter(Boolean),S={甲:{甲:"비견",乙:"겁재",丙:"식신",丁:"상관",戊:"편재",己:"정재",庚:"편관",辛:"정관",壬:"편인",癸:"정인"},乙:{甲:"겁재",乙:"비견",丙:"상관",丁:"식신",戊:"정재",己:"편재",庚:"정관",辛:"편관",壬:"정인",癸:"편인"},丙:{甲:"편인",乙:"정인",丙:"비견",丁:"겁재",戊:"식신",己:"상관",庚:"편재",辛:"정재",壬:"편관",癸:"정관"},丁:{甲:"정인",乙:"편인",丙:"겁재",丁:"비견",戊:"상관",己:"식신",庚:"정재",辛:"편재",壬:"정관",癸:"편관"},戊:{甲:"편관",乙:"정관",丙:"편인",丁:"정인",戊:"비견",己:"겁재",庚:"식신",辛:"상관",壬:"편재",癸:"정재"},己:{甲:"정관",乙:"편관",丙:"정인",丁:"편인",戊:"겁재",己:"비견",庚:"상관",辛:"식신",壬:"정재",癸:"편재"},庚:{甲:"편재",乙:"정재",丙:"편관",丁:"정관",戊:"편인",己:"정인",庚:"비견",辛:"겁재",壬:"식신",癸:"상관"},辛:{甲:"정재",乙:"편재",丙:"정관",丁:"편관",戊:"정인",己:"편인",庚:"겁재",辛:"비견",壬:"상관",癸:"식신"},壬:{甲:"식신",乙:"상관",丙:"편재",丁:"정재",戊:"편관",己:"정관",庚:"편인",辛:"정인",壬:"비견",癸:"겁재"},癸:{甲:"상관",乙:"식신",丙:"정재",丁:"편재",戊:"정관",己:"편관",庚:"정인",辛:"편인",壬:"겁재",癸:"비견"}},W={비견:"#00D2FF",겁재:"#6B82F9",식신:"#4CAF50",상관:"#81C784",편재:"#FF9800",정재:"#FFB74D",편관:"#F44336",정관:"#EF9A9A",편인:"#CE93D8",정인:"#BA68C8"},oe={비견:"독립심·경쟁심",겁재:"추진력·욕망",식신:"표현력·식복",상관:"재능·자유",편재:"사업·활동성",정재:"안정적 재물",편관:"권력·결단",정관:"명예·원칙",편인:"직관·학문",정인:"학습·보호"},b=o=>p&&S[p]&&S[p][o]||"",k=[{label:"시(時)",gan:n,ji:l,ss:b(n)},{label:"일(日)",gan:p,ji:t,ss:"일원(日元)"},{label:"월(月)",gan:m,ji:z,ss:b(m)},{label:"년(年)",gan:$,ji:w,ss:b($)}],_={};k.forEach(o=>{o.ss&&o.ss!=="일원(日元)"&&(_[o.ss]=(_[o.ss]||0)+1)});const F=`
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;text-align:center;">
        ...
        <tr>${k.map(o=>`<td style="padding:10px 4px;font-size:13px;font-weight:800;color:${o.ss==="일원(日元)"?"rgba(255,255,255,0.25)":W[o.ss]||"#fff"};">${o.ss}</td>`).join("")}</tr>
        <tr>${k.map(o=>`<td style="padding:4px;font-size:10px;color:rgba(255,255,255,0.35);line-height:1.4;">${o.ss==="일원(日元)"?"나 자신":oe[o.ss]||""}</td>`).join("")}</tr>
      </table>
    </div>
    <div style="margin-top:12px;display:flex;flex-wrap:wrap;gap:6px;">
      ${Object.entries(_).sort((o,fe)=>fe[1]-o[1]).map(([o,fe])=>`<span style="background:rgba(255,255,255,0.05);border:1px solid ${W[o]||"#888"}44;border-radius:20px;padding:4px 11px;font-size:12px;"><span style="color:${W[o]||"#fff"};font-weight:700;">${o}</span><span style="color:rgba(255,255,255,0.35);"> ×${fe}</span></span>`).join("")}
    </div>
  </div>`,C={申:"A",子:"A",辰:"A",寅:"B",午:"B",戌:"B",亥:"C",卯:"C",未:"C",巳:"D",酉:"D",丑:"D"},u=C[w]||C[t],V={A:"酉",B:"卯",C:"子",D:"午"},T={A:"寅",B:"申",C:"巳",D:"亥"},te={A:"辰",B:"戌",C:"未",D:"丑"},Z={甲:["丑","未"],戊:["丑","未"],庚:["丑","未"],乙:["子","申"],己:["子","申"],丙:["亥","酉"],丁:["亥","酉"],壬:["卯","巳"],癸:["卯","巳"],辛:["午","寅"]},H={甲:"卯",丙:"午",戊:"午",庚:"酉",壬:"子",乙:"辰",丁:"未",己:"未",辛:"戌",癸:"丑"},I=[j.includes(V[u])&&{name:"도화살(桃花殺)",icon:"🌸",color:"#FF6B9D",desc:"매력·인기운이 강합니다. 이성 관계와 예술·연예 분야에 유리합니다."},j.includes(T[u])&&{name:"역마살(驛馬殺)",icon:"🐴",color:"#FF9800",desc:"활동성이 강하고 변화가 많습니다. 해외·출장·이사 등 이동수가 있습니다."},(Z[p]||[]).some(o=>j.includes(o))&&{name:"천을귀인(天乙貴人)",icon:"⭐",color:"#FFD700",desc:"귀인의 도움을 받는 길성입니다. 위기 때 구원자가 나타납니다."},j.includes(H[p])&&{name:"양인살(羊刃殺)",icon:"⚔️",color:"#F44336",desc:"강한 승부욕·의지력. 칼날 같은 기운으로 신중함이 필요합니다."},j.includes(te[u])&&{name:"화개살(華蓋殺)",icon:"🔮",color:"#9C27B0",desc:"예술·종교·철학적 소질이 있습니다. 고독하지만 깊은 내면을 가집니다."}].filter(Boolean),i=I.some(o=>o.name.includes("역마살")),ee=I.some(o=>o.name.includes("천을귀인")),re=i&&ee?`<div style="margin-top:14px;background:linear-gradient(135deg,rgba(255,215,0,0.08),rgba(255,152,0,0.05));border:1px solid rgba(255,215,0,0.25);border-radius:12px;padding:14px;">
  ...
  </div>`,y={子:["壬","癸"],丑:["癸","辛","己"],寅:["戊","丙","甲"],卯:["甲","乙"],辰:["乙","癸","戊"],巳:["戊","庚","丙"],午:["丙","己","丁"],未:["丁","乙","己"],申:["戊","壬","庚"],酉:["庚","辛"],戌:["辛","丁","戊"],亥:["戊","甲","壬"]},ne=["여기(餘氣)","중기(中氣)","정기(正氣)"],pe={甲:"목",乙:"목",丙:"화",丁:"화",戊:"토",己:"토",庚:"금",辛:"금",壬:"수",癸:"수"},ge={목:"#4CAF50",화:"#FF5722",토:"#FF9800",금:"#9E9E9E",수:"#2196F3"},se=`
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;">
      ${[{label:"시(時)",ji:l},{label:"일(日)",ji:t},{label:"월(月)",ji:z},{label:"년(年)",ji:w}].map(o=>{const fe=y[o.ji]||[];return`<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:10px 8px;text-align:center;">
          <div style="font-size:12px;color:rgba(255,255,255,0.65);margin-bottom:6px;">${o.label} · ${o.ji||"?"}</div>
          ${fe.map((Fe,Se)=>{const Me=pe[Fe]||"토",Oe=ge[Me]||"#aaa",Ie=b(Fe),De=Se===fe.length-1?"정기(正氣)":ne[Se]||"";return`<div style="margin-bottom:5px;">
              <span style="font-size:15px;font-weight:800;color:${Oe};">${Fe}</span>
              <span style="font-size:11px;color:rgba(255,255,255,0.6);display:block;line-height:1.3;">${De}</span>
              ${Ie?`<span style="font-size:9px;color:${W[Ie]||"#aaa"};font-weight:600;">${Ie}</span>`:""}
            </div>`}).join("")}
          ${fe.length===0?'<div style="color:rgba(255,255,255,0.45);font-size:11px;">—</div>':""}
        </div>`}).join("")}
    </div>
    <div style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);">💡 정기(正氣)가 가장 강한 핵심 에너지, 여기는 전 계절의 잔여 기운입니다.</div>
  </div>`,me=["甲","乙","丙","丁","戊","己","庚","辛","壬","癸"],r=["子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥"],d=[["戌","亥"],["申","酉"],["午","未"],["辰","巳"],["寅","卯"],["子","丑"]],x={子:"자",丑:"축",寅:"인",卯:"묘",辰:"진",巳:"사",午:"오",未:"미",申:"신",酉:"유",戌:"술",亥:"해"},M=me.indexOf(p),v=r.indexOf(t);let g=M;for(;g%12!==v&&g<60;)g+=10;const P=Math.floor(g/10),X=d[P]||[],K=[l,t,z,w].filter(Boolean),q=X.filter(o=>K.includes(o)),O={戌:"술(戌) 공망 — 토(土)·믿음·종교·부동산 영역에서 허무함을 느낄 수 있습니다.",亥:"해(亥) 공망 — 수(水)·지혜·철학·여행 분야에서 노력 대비 결실이 약할 수 있습니다.",申:"신(申) 공망 — 금(金)·재물·법·조직 영역에서 공허함이 생깁니다.",酉:"유(酉) 공망 — 금(金)·명예·이성 인연·결실 분야가 채워지지 않는 느낌이 있습니다.",午:"오(午) 공망 — 화(火)·명성·열정·중년의 성취가 기대보다 약할 수 있습니다.",未:"미(未) 공망 — 토(土)·가정·전통·안정감에서 결핍을 느낍니다.",辰:"진(辰) 공망 — 토(土)·사업·현실 기반·실속이 흔들릴 수 있습니다.",巳:"사(巳) 공망 — 화(火)·계획·지식·전문성 영역에서 허탕을 치기 쉽습니다.",寅:"인(寅) 공망 — 목(木)·시작·도전·형제 인연이 공허합니다.",卯:"묘(卯) 공망 — 목(木)·창의력·학문·이성 인연이 약해집니다.",子:"자(子) 공망 — 수(水)·지혜·부하·자녀 인연에서 결핍이 생깁니다.",丑:"축(丑) 공망 — 토(土)·재고·비밀·말년 안정이 불안할 수 있습니다."},ce=`
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="display:flex;gap:10px;align-items:center;margin-bottom:12px;flex-wrap:wrap;">
      ...
      <div style="flex:1;min-width:120px;">
        ${q.length>0?`<div style="font-size:12px;font-weight:700;color:#FF8A80;margin-bottom:4px;">⚠️ 원국에 공망 해당 글자 있음</div>
             <div style="font-size:11px;color:rgba(255,255,255,0.55);">${q.map(o=>x[o]).join("·")} 자리가 구멍납니다. 이 영역에서 노력해도 공허함이 생기는 원인입니다.</div>`:`<div>
    ...
    <div style="display:flex;flex-direction:column;gap:8px;">
      ${X.map(o=>`<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:10px 12px;font-size:12px;color:rgba(255,255,255,0.65);line-height:1.7;">${O[o]||""}</div>`).join("")}
    </div>
  ...
  </div>`,Y=[n,p,m,$].filter(Boolean),N=[l,t,z,w].filter(Boolean),ae=[{a:"甲",b:"己",result:"土합",desc:"갑기합(甲己合) — 리더십(木)과 포용력(土)이 결합, 안정적 추진력을 발휘합니다. 공직·기업 경영에 이상적입니다."},{a:"乙",b:"庚",result:"金합",desc:"을경합(乙庚合) — 유연함(木)과 단호함(金)이 결합, 협상력과 결단력을 동시에 갖춥니다."},{a:"丙",b:"辛",result:"水합",desc:"병신합(丙辛合) — 열정(火)과 예리함(金)이 수(水)로 변환, 전략적 분석력과 깊은 통찰이 생깁니다."},{a:"丁",b:"壬",result:"木합",desc:"정임합(丁壬合) — 감수성(火)과 지혜(水)가 목(木)으로 변환, 창의적 영감과 인문학적 재능이 발달합니다."},{a:"戊",b:"癸",result:"火합",desc:"무계합(戊癸合) — 현실감(土)과 직관(水)이 화(火)로 변환, 열정적 행동력과 카리스마가 형성됩니다."}].filter(o=>Y.includes(o.a)&&Y.includes(o.b)),je=[{group:["申","子","辰"],result:"水局",desc:"수(水) 기운이 강하게 형성"},{group:["巳","酉","丑"],result:"金局",desc:"금(金) 기운이 강하게 형성"},{group:["寅","午","戌"],result:"火局",desc:"화(火) 기운이 강하게 형성"},{group:["亥","卯","未"],result:"木局",desc:"목(木) 기운이 강하게 형성"}].map(o=>({cnt:o.group.filter(Fe=>N.includes(Fe)).length,...o})).filter(o=>o.cnt>=2),we=[{a:"子",b:"丑",result:"土합",desc:"자축합(子丑合) — 수(水)와 토(土)가 결합해 단단한 땅을 형성합니다. 신용·자산 기반이 흔들림 없이 지켜지는 강한 결속력입니다."},{a:"寅",b:"亥",result:"木합",desc:"인해합(寅亥合) — 목(木) 기운이 강화됩니다. 시작과 성장의 에너지가 서로를 북돋아 새로운 도전에 유리합니다."},{a:"卯",b:"戌",result:"火합",desc:"묘술합(卯戌合) — 화(火) 기운으로 변합니다. 따뜻한 정(情)과 열정이 관계를 불태우지만, 집착이 될 수 있어 균형이 필요합니다."},{a:"辰",b:"酉",result:"金합",desc:"진유합(辰酉合) — 금(金) 기운이 강해집니다. 현실적 판단력과 재물 결실이 단단하게 쌓이는 실속형 결합입니다."},{a:"巳",b:"申",result:"水합",desc:"사신합(巳申合) — 수(水) 기운으로 변합니다. 지혜·전략·소통이 강화되나, 화(火)와 금(金)의 긴장이 내재합니다."},{a:"午",b:"未",result:"土합",desc:"오미합(午未合) — 토(土) 기운이 두터워집니다. 중심을 잡아주는 안정 에너지로, 중년 이후 자산 축적에 유리합니다."}].filter(o=>N.includes(o.a)&&N.includes(o.b)),$e=[{a:"子",b:"午",desc:"子午충 — 수화(水火) 충돌, 변동·이동수 강함"},{a:"丑",b:"未",desc:"丑未충 — 토토(土土) 충돌, 직장·부동산 변동"},{a:"寅",b:"申",desc:"寅申충 — 목금(木金) 충돌, 충동적 행동 주의"},{a:"卯",b:"酉",desc:"卯酉충 — 목금(木金) 충돌, 이성 관계·인간관계 마찰"},{a:"辰",b:"戌",desc:"辰戌충 — 토토(土土) 충돌, 기반·환경의 급변"},{a:"巳",b:"亥",desc:"巳亥충 — 화수(火水) 충돌, 사업·건강의 기복"}].filter(o=>N.includes(o.a)&&N.includes(o.b)),le=`
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="margin-bottom:12px;">
      ...
      ${ae.map(o=>`<div style="background:rgba(0,210,255,0.05);border:1px solid rgba(0,210,255,0.15);border-radius:10px;padding:12px;margin-bottom:6px;">
    ...
    <div style="margin-bottom:12px;">
      ...
      ${je.map(o=>`<div style="background:rgba(76,175,80,0.05);border:1px solid rgba(76,175,80,0.18);border-radius:10px;padding:10px 12px;margin-bottom:6px;font-size:12px;color:rgba(255,255,255,0.75);">
        <span style="font-size:13px;font-weight:800;color:#AED581;margin-right:8px;">${o.group.filter(fe=>N.includes(fe)).join("·")} ${o.cnt===2?"반삼합":"삼합"}</span>${o.desc} <span style="color:rgba(255,255,255,0.4);">(→ ${o.result})</span>
    ...
    <div style="margin-bottom:12px;">
      ...
      ${we.map(o=>`<div style="background:rgba(156,39,176,0.05);border:1px solid rgba(156,39,176,0.18);border-radius:10px;padding:12px;margin-bottom:6px;">
  ...
  <div class="saju-card" style="margin-bottom:16px;">
    ...
    <div style="margin-bottom:12px;">
      ...
      ${xe.map(o=>`<div style="background:rgba(244,67,54,0.05);border:1px solid rgba(244,67,54,0.2);border-radius:10px;padding:12px;margin-bottom:6px;">
    ...
    <div>
      ...
      ${Be.map(o=>`<div style="background:rgba(156,39,176,0.05);border:1px solid rgba(156,39,176,0.2);border-radius:10px;padding:12px;margin-bottom:6px;">
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:flex;gap:12px;align-items:flex-start;">
      ...
      <div style="flex:1;">
        ${l.map((U,xe)=>`<div style="display:flex;gap:8px;align-items:flex-start;${xe<l.length-1?"margin-bottom:10px":""}"><span style="color:#00d2ff;font-size:13px;flex-shrink:0;margin-top:2px;">${["①","②","③"][xe]||"✦"}</span><span style="font-size:13px;color:rgba(255,255,255,0.82);line-height:1.75;">${U}</span></div>`).join("")}
      </div>
    </div>
  </div>`,ge=`
  <div class="saju-card" style="${B}">
    ${y("💼","나에게 맞는 직업")}
    <div style="font-size:13px;font-weight:700;color:#81C784;margin-bottom:10px;">${oe.icon} ${oe.title}</div>
    <div style="margin-bottom:12px;">${oe.jobs.map(U=>ne(U,"rgba(76,175,80,0.1)","rgba(76,175,80,0.3)","#81C784")).join("")}</div>
    <div style="background:rgba(76,175,80,0.05);border:1px solid rgba(76,175,80,0.15);border-radius:10px;padding:10px 14px;font-size:12px;color:rgba(255,255,255,0.5);">💡 ${oe.tip}</div>
  </div>`,ue=`
  <div class="saju-card" style="${B}">
    ${y("💰","재물운 스타일")}
    <div style="background:rgba(255,152,0,0.08);border:1px solid rgba(255,152,0,0.25);border-radius:12px;padding:14px;margin-bottom:10px;">
      <div style="font-size:16px;font-weight:800;color:#FFB74D;margin-bottom:8px;">${F}</div>
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      <div style="background:rgba(244,67,54,0.06);border:1px solid rgba(244,67,54,0.18);border-radius:12px;padding:12px;">
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.7);line-height:1.7;">${H}</div>
      </div>
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="background:rgba(156,204,101,0.08);border:1px solid rgba(156,204,101,0.2);border-radius:12px;padding:14px;">
      <div style="font-size:13px;font-weight:800;color:#AED581;margin-bottom:10px;">📌 ${re.arrow}</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.78);line-height:1.8;">${re.tip}</div>
    </div>
  </div>`,r=`
  <div class="saju-card" style="${B}">
    ${y("🍀","2026년 나의 운세")}
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      <div style="background:rgba(0,200,100,0.05);border:1px solid rgba(0,200,100,0.2);border-radius:12px;padding:14px;">
        ...
        ${L.do_.map(U=>`<div style="display:flex;gap:6px;align-items:flex-start;margin-bottom:8px;"><span style="color:#69F0AE;flex-shrink:0;font-size:11px;margin-top:2px;">▸</span><span style="font-size:12px;color:rgba(255,255,255,0.78);line-height:1.65;">${U}</span></div>`).join("")}
      </div>
      <div style="background:rgba(255,80,80,0.05);border:1px solid rgba(255,80,80,0.2);border-radius:12px;padding:14px;">
        ...
        ${L.dont.map(U=>`<div style="display:flex;gap:6px;align-items:flex-start;margin-bottom:8px;"><span style="color:#FF8A80;flex-shrink:0;font-size:11px;margin-top:2px;">▸</span><span style="font-size:12px;color:rgba(255,255,255,0.78);line-height:1.65;">${U}</span></div>`).join("")}
      </div>
    </div>
  ...
  </div>`:"",P=((ke=e.daYun)==null?void 0:ke.list)||[],X=P.findIndex(U=>U.isCurrent),K=P[X],q=X>=0?P[X+1]:null,O=q?q.startAge:null,ce=O?O-2:null,Y=O?O+2:null,N=new Date().getFullYear(),ie=(a==null?void 0:a.year)||N,ae=N-ie,be=O!==null&&Math.abs(ae-O)<=3,je=O?`
  <div class="saju-card" style="${B}">
    ...
    <div style="background:rgba(255,193,7,0.07);border:1px solid rgba(255,193,7,0.22);border-radius:12px;padding:14px;margin-bottom:12px;">
      ...
      <div style="font-size:13px;color:rgba(255,255,255,0.85);line-height:1.7;">
        <strong style="color:#FFD54F;">만 ${ce}세~${Y}세</strong> 구간이 교운기(대운 환절기)입니다.
        ${K?`현재 <strong style="color:#00D2FF;">${K.ganZhi}</strong> 대운에서 `:""}
        ${q?`<strong style="color:#69F0AE;">${q.ganZhi}</strong> 대운으로 기운이 전환됩니다.`:""}
      </div>
    </div>
  ...
  <div class="saju-card" style="${B}">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;">
      <div style="background:linear-gradient(135deg,rgba(0,210,255,0.06),rgba(107,130,249,0.04));border:1px solid rgba(0,210,255,0.2);border-radius:12px;padding:14px;">
        ...
        <div style="font-size:18px;font-weight:900;color:#00D2FF;margin-bottom:6px;letter-spacing:1px;">${we.guiIn.jilju}</div>
        <div style="font-size:12px;color:rgba(255,255,255,0.72);line-height:1.8;">${we.guiIn.reason}</div>
      </div>
      <div style="background:linear-gradient(135deg,rgba(105,240,174,0.06),rgba(0,200,83,0.04));border:1px solid rgba(105,240,174,0.2);border-radius:12px;padding:14px;">
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.72);line-height:1.8;">${we.energy.desc}</div>
  ...
  </div>`:"";return pe+ge+ue+se+me+r+d+g+je+Ce}function We(e,a,f,A){var Z,H,I,i,ee,re,ve,G;window._gunghamContext=null,window._sajuContext={eightChar:e.eightChar,distribution:e.distribution,weak:e.weak,daYun:e.daYun,samjae:e.samjae,birthInput:a,temples:((Z=f==null?void 0:f.results)==null?void 0:Z.slice(0,3).map(c=>{var h,L;return{name:(h=c.temple)==null?void 0:h.name,address:(L=c.temple)==null?void 0:L.address}}))||[]};const E=document.getElementById("results");E.classList.remove("hidden");const p=document.getElementById("match-form");p&&(p.style.display="none");const m={木:"목",火:"화",土:"토",金:"금",水:"수"},$={목:"#4CAF50",화:"#FF5722",토:"#FF9800",금:"#9E9E9E",수:"#2196F3"},n={목:"木 목",화:"火 화",토:"土 토",금:"金 금",수:"水 수"},t=e.eightChar,z=t?(()=>{const c=[{label:"시(時)",char:t.time,wx:t.timeWx},{label:"일(日)",char:t.day,wx:t.dayWx},{label:"월(月)",char:t.month,wx:t.monthWx},{label:"년(年)",char:t.year,wx:t.yearWx}],h=y=>y?y[0]:"",L=y=>y?y[1]:"",B=c.map(y=>{const ne=[...y.wx||""].map(ge=>m[ge]).filter(Boolean),pe=$[ne[0]]||"#00d2ff";return{chars:ne,col:pe}});return`
    <table class="saju-table">
      ...
      <tbody>
        <tr class="saju-table-gan">
          ${c.map((y,ne)=>`<td style="color:${B[ne].col}">${h(y.char)}</td>`).join("")}
        </tr>
        <tr class="saju-table-ji">
          ${c.map((y,ne)=>`<td style="color:${B[ne].col}">${L(y.char)}</td>`).join("")}
        </tr>
  ...
  `).join(""),j=a.calendarType==="lunar"?"음력":"양력",S=`${a.hour}시 ${a.minute>0?a.minute+"분":""}`.trim();a.birthLongitude&&`${a.birthLongitude}`;const W=((H=e.weak)==null?void 0:H.부족오행)??"",oe={목:"#4CAF50",화:"#FF5722",토:"#FF9800",금:"#9E9E9E",수:"#2196F3"},b={목:"木",화:"火",토:"土",금:"金",수:"水"},k=(f==null?void 0:f.results)||[],_=k.length>0?`
    <div class="saju-dist-section" style="border-color:rgba(0,210,255,0.3);">
      <div class="saju-dist-title">🏯 나의 인연사찰 추천
        ${W?`<span style="font-size:12px;font-weight:400;color:${oe[W]||"#00d2ff"};margin-left:8px;">· ${W}(${b[W]||""}) 기운 보완</span>`:""}
      </div>
      ${k.slice(0,5).map((c,h)=>{var y;const L=c.temple||{},B=(y=c.detail)==null?void 0:y.distanceKm;return`
    ...
    </div>`:'<div class="saju-notice-banner" style="text-align:center;">📍 위치를 확인하는 중이었거나 사찰 데이터를 불러오지 못했습니다.<br><button class="submit-btn" style="margin-top:12px;padding:10px 24px;" id="retry-match-btn">🔄 인연사찰 다시 찾기</button></div>',F=`
    <div class="saju-card" style="margin-bottom:0;">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:12px;justify-content:center;">
        ${l?`<details style="width:100%;"><summary style="cursor:pointer;font-size:12px;color:rgba(255,255,255,0.4);list-style:none;">▼ 오행 분포 보기</summary><div style="margin-top:8px;">${l}</div></details>`:""}
            ...
            </div>`}).join("")}`,e.samjae&&`${((I=e.samjae)==null?void 0:I.animalsKo)||""}${e.samjae.samjaeTarget}${e.samjae.groups.map(c=>{const h=new Date().getFullYear(),L=c.some(B=>B.year===h||B.year===h-1||B.year===h+1);return`<div class="samjae-group ${L?"samjae-active":""}">
    ...
    <div class="saju-page-wrap">
      ...
      ${rt(e)}
      ${st(e,a)}
      <div id="saju-noprint-bottom">${_}${F}</div>
    </div>`,E.scrollIntoView({behavior:"smooth",block:"start"}),(re=document.getElementById("saju-go-home"))==null||re.addEventListener("click",()=>{var h;E.classList.add("hidden"),E.innerHTML="";const c=document.getElementById("match-form");c&&(c.style.display=""),(h=document.getElementById("app"))==null||h.scrollIntoView({behavior:"smooth",block:"start"})}),(ve=document.getElementById("saju-download-btn"))==null||ve.addEventListener("click",()=>{var me,r;const c=a.calendarType==="lunar"?"음력":"양력",h=`${a.hour}시${a.minute>0?" "+a.minute+"분":""}`,L=`사주 풀이 — ${c} ${a.year}년 ${a.month}월 ${a.day}일 ${h}`,B=(f==null?void 0:f.results)||[],y=B.slice(0,5).map((d,x)=>{var g;const M=d.temple||{},v=(g=d.detail)==null?void 0:g.distanceKm;return`<div class="p-temple"><span class="p-rank">${x+1}위</span> <strong>${M.name||""}</strong><br>
       <span class="p-addr">${M.address||""}</span>${v!=null?" · "+(v<1?(v*1e3).toFixed(0)+"m":v.toFixed(1)+"km"):""}
       &nbsp;${d.score||""}점</div>`}).join(""),ne=e.eightChar?[{l:"시(時)",c:e.eightChar.time,w:e.eightChar.timeWx},{l:"일(日)",c:e.eightChar.day,w:e.eightChar.dayWx},{l:"월(月)",c:e.eightChar.month,w:e.eightChar.monthWx},{l:"년(年)",c:e.eightChar.year,w:e.eightChar.yearWx}]:[],pe={목:"#2e7d32",화:"#c62828",토:"#e65100",금:"#455a64",수:"#1565c0"},ge={木:"목",火:"화",土:"토",金:"금",水:"수"},ue=ne.length?`
      <table class="p-table">
        <tr><th>구분</th>${ne.map(d=>`<th>${d.l}</th>`).join("")}</tr>
        <tr><td>천간</td>${ne.map(d=>{var v;const x=[...d.w].map(g=>ge[g]).filter(Boolean);return`<td style="color:${pe[x[0]]||"#333"};font-size:26px;font-weight:900;">${((v=d.c)==null?void 0:v[0])||""}</td>`}).join("")}</tr>
        <tr><td>지지</td>${ne.map(d=>{var v;const x=[...d.w].map(g=>ge[g]).filter(Boolean);return`<td style="color:${pe[x[0]]||"#333"};font-size:26px;font-weight:900;">${((v=d.c)==null?void 0:v[1])||""}</td>`}).join("")}</tr>
        <tr><td>오행</td>${ne.map(d=>`<td style="font-size:12px;">${[...d.w].map(M=>ge[M]).filter(Boolean).join("·")}</td>`).join("")}</tr>
      </table>`:"",se=(r=(me=e.daYun)==null?void 0:me.list)==null?void 0:r.find(d=>d.isCurrent);`${L}${L}${new Date().toLocaleDateString("ko-KR")}${ue}`,B.length&&`${y}`,`${C.replace(/class="saju-explain-heading"/g,'class="explain-heading"')}`,se&&`${e.daYun.direction}${e.daYun.startAge}${e.daYun.list.map(d=>`
          <div class="dayun-chip ${d.isCurrent?"current":""}">
            ${d.isCurrent?"▶ ":""}${d.startAge}세 ${d.ganZhi}
          </div>`).join("")}`,window.print()}),(G=document.getElementById("retry-match-btn"))==null||G.addEventListener("click",async()=>{const c=document.getElementById("retry-match-btn");c.textContent="탐색 중...",c.disabled=!0;try{const{userLat:h,userLng:L}=await Ne(),y=await(await fetch("/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({birthInput:a,userLat:h,userLng:L,purpose:"healing"})})).json();y.error||We(e,a,y,A)}catch{c.textContent="🔄 다시 찾기",c.disabled=!1}})}function Le(e,a,f,A){var me;const E=document.getElementById("results");E.classList.remove("hidden");const p=document.getElementById("match-form");p&&(p.style.display="none");const m=e.temple||{},$=e.detail||{},n=e.score||0,t={목:{c:"#4ADE80",r:"74,222,128"},화:{c:"#FB923C",r:"251,146,60"},토:{c:"#FACC15",r:"250,204,21"},금:{c:"#F0C060",r:"240,192,96"},수:{c:"#38BDF8",r:"56,189,248"}},z=t[$.templeOhaeng]||t.금,w=z.c,l=z.r,j={info:{c:"#60A5FA",r:"96,165,250"},loc:{c:"#F87171",r:"248,113,113"},ohaeng:{c:w,r:l},reason:{c:"#C084FC",r:"192,132,252"},purpose:{c:w,r:l},guide:{c:"#34D399",r:"52,211,153"},gido:{c:"#FCD34D",r:"252,211,77"},date:{c:"#818CF8",r:"129,140,248"},dist:{c:w,r:l}};function S(r,d,x){return'<div class="ds-card" style="background:rgba('+r.r+",0.06);border:1px solid rgba("+r.r+",0.22);border-left:4px solid "+r.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;"><div style="font-size:14px;font-weight:800;color:'+r.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+r.r+',0.2);letter-spacing:.05em;">'+d+'</div><div style="font-size:14px;color:#E2E8F0;line-height:1.9;">'+x+"</div></div>"}const W={목:"🌿",화:"🔥",토:"🌍",금:"✨",수:"💧"},oe={목:"성장·창의·도전",화:"열정·명예·활력",토:"안정·신뢰·중심",금:"결실·의지·재물",수:"지혜·학업·직관"},b={목:"새로운 일을 시작하거나 창의적인 활동에 힘을 불어넣어 줍니다.",화:"이름을 알리고 사람들 사이에서 빛나게 해주는 기운입니다.",토:"흔들리는 마음을 안정시키고 근본을 다지게 해줍니다.",금:"결실을 맺고 재물과 의지력을 강화시켜 줍니다.",수:"지혜와 직관을 높여주고 학업·시험에 도움이 됩니다."},k=W[$.templeOhaeng]||"✦",_=a?a.distribution||{}:{},F=a&&a.purpose||"",C=a?a.purposeGuide||[]:[],u=a?a.recommendedDates||[]:[],V=f?u.length:Math.min(3,u.length),T=m.history||"",te=f?T:T.length>120?T.slice(0,120)+'… <span style="color:rgba(255,255,255,0.35);font-size:12px;">🔒 전체보기는 멤버 전용</span>':T||"정보 없음",Z="https://map.naver.com/v5/search/"+encodeURIComponent((m.name||"")+" "+(m.address||"")),H=$.distanceKm!=null?$.distanceKm<1?Math.round($.distanceKm*1e3)+"m":$.distanceKm.toFixed(1)+"km":"정보 없음",I=e.weather?'<div style="display:inline-flex;align-items:center;gap:7px;background:rgba(56,189,248,0.15);border:1px solid rgba(56,189,248,0.4);border-radius:20px;padding:5px 14px;font-size:13px;color:#7DD3FA;margin-top:10px;">🌤 <b>'+e.weather.condition+"</b>&nbsp;"+e.weather.temp+"°C</div>":"";let i='<div class="temple-detail-page">';i+="<style>.ds-card{transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease;cursor:default;}.ds-card:hover{transform:translateY(-3px);box-shadow:0 8px 32px rgba(0,0,0,.35);border-color:rgba(255,255,255,0.28) !important;}.ds-card:hover .ds-title{letter-spacing:.07em;}#detail-back-btn:hover{background:rgba(255,255,255,0.16);}.detail-date-card{transition:transform .15s,box-shadow .15s;}.detail-date-card:hover{transform:translateY(-4px) scale(1.06);box-shadow:0 8px 24px rgba(0,0,0,.4);}</style>",i+='<button id="detail-back-btn" style="display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.2);color:rgba(255,255,255,0.8);border-radius:20px;padding:8px 18px;font-size:13px;font-weight:600;cursor:pointer;margin-bottom:18px;transition:background .15s;">← 목록으로</button>',i+='<div id="detail-hero" style="position:relative;overflow:hidden;background:linear-gradient(160deg,rgba('+l+",0.18) 0%,rgba(0,0,0,0) 70%);border:1px solid rgba("+l+",0.35);border-top:3px solid "+w+';border-radius:22px;padding:28px 22px 22px;margin-bottom:16px;text-align:center;">',i+='<div id="detail-hero-img" style="position:absolute;inset:0;background-size:cover;background-position:center top;border-radius:22px;opacity:0;transition:opacity .6s ease;"></div>',i+='<div id="detail-hero-overlay" style="position:absolute;inset:0;background:linear-gradient(180deg,rgba(5,10,25,0.55) 0%,rgba(5,10,25,0.82) 100%);border-radius:22px;opacity:0;transition:opacity .6s ease;"></div>',i+='<div style="position:relative;z-index:1;">',i+='<div style="font-size:11px;letter-spacing:.2em;color:'+w+';text-transform:uppercase;margin-bottom:8px;font-weight:700;opacity:.9;">'+k+"  "+($.templeOhaeng||"")+"(오행) · "+(oe[$.templeOhaeng]||"")+"</div>",i+='<div style="font-size:clamp(28px,5vw,40px);font-weight:900;color:#fff;margin-bottom:6px;letter-spacing:-.5px;text-shadow:0 0 30px rgba('+l+',.4);">'+(m.name||"사찰")+"</div>",m.foundedYear&&(i+='<div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">창건 '+m.foundedYear+"년</div>"),i+=I,i+='<div style="margin-top:18px;">',i+='<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:6px;"><span style="color:rgba(255,255,255,0.45);">인연 매칭 점수</span><span style="color:'+w+';font-weight:800;font-size:15px;">'+n+"점</span></div>",i+='<div style="height:10px;background:rgba(0,0,0,0.4);border-radius:5px;overflow:hidden;">',i+='<div style="height:100%;width:'+Math.min(n,100)+"%;background:linear-gradient(90deg,"+w+" 0%,#fff 120%);border-radius:5px;box-shadow:0 0 12px rgba("+l+',.7);"></div>',i+="</div></div></div></div>",i+='<div id="temple-gallery" style="display:none;margin-bottom:14px;"><div style="font-size:13px;font-weight:800;color:rgba(255,255,255,0.5);margin-bottom:8px;letter-spacing:.08em;">📷 사찰 사진</div><div id="gallery-scroll" style="display:flex;gap:10px;overflow-x:auto;padding-bottom:8px;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,0.15) transparent;"></div></div>';const ee=j.info;i+='<div class="ds-card" style="background:rgba('+ee.r+",0.06);border:1px solid rgba("+ee.r+",0.22);border-left:4px solid "+ee.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+ee.c+";margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid rgba("+ee.r+',0.2);letter-spacing:.05em;">🏯 사찰 기본 정보</div>',i+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">',i+='<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;">',i+='<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:6px;letter-spacing:.1em;">🧭 방위</div>',i+='<div style="font-size:16px;font-weight:800;color:#E2E8F0;">'+($.bearing||"—")+"</div></div>",i+='<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;">',i+='<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:6px;letter-spacing:.1em;">📏 거리</div>',i+='<div style="font-size:16px;font-weight:800;color:#E2E8F0;">'+H+"</div></div>",i+='<div style="background:rgba('+l+",0.1);border:1px solid rgba("+l+',0.35);border-radius:12px;padding:14px;">',i+='<div style="font-size:11px;color:rgba('+l+',.6);margin-bottom:6px;letter-spacing:.1em;">'+k+" 오행 기운</div>",i+='<div style="font-size:15px;font-weight:800;color:'+w+';">'+($.templeOhaeng||"—")+" · "+(oe[$.templeOhaeng]||"—")+"</div></div>",i+='<div style="background:rgba('+l+",0.1);border:1px solid rgba("+l+',0.35);border-radius:12px;padding:14px;">',i+='<div style="font-size:11px;color:rgba('+l+',.6);margin-bottom:6px;letter-spacing:.1em;">⭐ 매칭 점수</div>',i+='<div style="font-size:16px;font-weight:800;color:'+w+';">'+n+"점</div></div>",i+="</div></div>",function(){var r=(m.history||"")+" "+(m.name||""),d=[];/조계종/.test(r)?d.push({icon:"⛩️",label:"대한불교 조계종",color:"#34D399",r:"52,211,153"}):/태고종/.test(r)?d.push({icon:"⛩️",label:"한국불교 태고종",color:"#818CF8",r:"129,140,248"}):/천태종/.test(r)?d.push({icon:"⛩️",label:"천태종",color:"#38BDF8",r:"56,189,248"}):/진각종/.test(r)&&d.push({icon:"⛩️",label:"진각종",color:"#FB923C",r:"251,146,60"});var x=null;if(m.foundedYear){var M=parseInt(m.foundedYear);M<668?x={icon:"🏺",label:"삼국시대 고찰",color:"#F472B6",r:"244,114,182"}:M<935?x={icon:"🏛️",label:"통일신라 고찰",color:"#FBBF24",r:"251,191,36"}:M<1392?x={icon:"🏯",label:"고려시대 고찰",color:"#FBBF24",r:"251,191,36"}:M<1897?x={icon:"🏯",label:"조선시대 사찰",color:"#94A3B8",r:"148,163,184"}:x={icon:"🕌",label:"근현대 사찰",color:"#94A3B8",r:"148,163,184"}}else/신라 (문무왕|진흥왕|경덕왕|흥덕왕|선덕왕|태종무열)/.test(r)?x={icon:"🏛️",label:"통일신라 고찰",color:"#FBBF24",r:"251,191,36"}:/고려/.test(r)&&!/조선/.test(r)?x={icon:"🏯",label:"고려시대 고찰",color:"#FBBF24",r:"251,191,36"}:/신라/.test(r)&&!/고려|조선/.test(r)?x={icon:"🏛️",label:"신라 고찰",color:"#FBBF24",r:"251,191,36"}:/조선/.test(r)&&(x={icon:"🏯",label:"조선시대 사찰",color:"#94A3B8",r:"148,163,184"});if(x&&d.push(x),/국보 제?\d+호|국보[가-힣 ]*제?\d+/.test(r)?d.push({icon:"🏆",label:"국보 보유",color:"#FBBF24",r:"251,191,36"}):/보물 제?\d+호|보물[가-힣 ]*제?\d+/.test(r)&&d.push({icon:"💎",label:"보물 보유",color:"#D4AF37",r:"212,175,55"}),/사적 제?\d+호|사적지/.test(r)&&d.push({icon:"📜",label:"사적지",color:"#94A3B8",r:"148,163,184"}),/원효대사|원효/.test(r)?d.push({icon:"🧘",label:"원효대사 창건",color:"#818CF8",r:"129,140,248"}):/의상대사|의상/.test(r)&&d.push({icon:"🧘",label:"의상대사 창건",color:"#818CF8",r:"129,140,248"}),/임진왜란|병자호란|의병/.test(r)&&d.push({icon:"🛡️",label:"호국불교",color:"#FB923C",r:"251,146,60"}),/본사/.test(r)&&d.push({icon:"🏯",label:"교구 본사",color:"#F472B6",r:"244,114,182"}),/템플스테이/.test(r)&&d.push({icon:"🌙",label:"템플스테이",color:"#38BDF8",r:"56,189,248"}),!!d.length){var v='<div style="display:flex;flex-wrap:wrap;gap:8px;">'+d.map(function(g){return'<span style="display:inline-flex;align-items:center;gap:5px;background:rgba('+g.r+",0.12);border:1px solid rgba("+g.r+",0.35);color:"+g.color+';border-radius:20px;padding:6px 14px;font-size:12px;font-weight:700;">'+g.icon+" "+g.label+"</span>"}).join("")+"</div>";i+=S({r:"148,163,184",c:"#94A3B8"},"🏷️ 사찰 특징",v)}}();const re=j.loc;if(i+='<div class="ds-card" style="background:rgba('+re.r+",0.06);border:1px solid rgba("+re.r+",0.22);border-left:4px solid "+re.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+re.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+re.r+',0.2);">📍 위치 &amp; 방문 안내</div>',m.address&&(i+='<div style="font-size:14px;color:#CBD5E1;margin-bottom:14px;line-height:1.7;">'+m.address+"</div>"),i+='<a href="'+Z+'" target="_blank" rel="noopener" style="display:block;text-align:center;background:rgba('+re.r+",0.12);border:1px solid rgba("+re.r+",0.35);color:"+re.c+';border-radius:12px;padding:12px;font-weight:700;font-size:14px;text-decoration:none;">🗺️ 네이버 지도에서 길찾기</a>',i+="</div>",$.templeOhaeng&&b[$.templeOhaeng]){var ve={목:{env:"🌿 동쪽 · 숲·계곡",season:"봄(3~5월) 방문 최적",color:"#4ADE80",r:"74,222,128",detail:"생명력과 성장의 기운이 충만한 도량입니다. 울창한 수목과 맑은 물소리가 어우러져 마음을 새롭게 열어주며, 새로운 시작·가정화목·창의적 도전에 특히 좋습니다."},화:{env:"☀️ 남쪽 · 양지·온기",season:"여름(6~8월) 방문 최적",color:"#F97316",r:"249,115,22",detail:"밝고 따뜻한 불꽃 기운이 살아 숨쉬는 도량입니다. 햇살이 가득한 남향 터에 자리잡아 좋은 인연·명예·활력을 끌어당기는 강한 화(火) 에너지가 흐릅니다."},토:{env:"🏔️ 중앙 · 산중·평지",season:"환절기(3·9월) 방문 최적",color:"#FACC15",r:"250,204,21",detail:"대지의 안정된 기운이 깊게 뿌리내린 도량입니다. 든든하고 포근한 토(土) 에너지가 흔들리는 심신을 안정시키고, 건강·치유·신뢰 회복에 알맞습니다."},금:{env:"🍂 서쪽 · 산자락·바위",season:"가을(9~11월) 방문 최적",color:"#D4AF37",r:"212,175,55",detail:"결실과 의지의 금(金) 기운이 가득한 도량입니다. 가을 수확처럼 노력에 합당한 결실을 맺게 해주며, 재물운·결단력·성취를 기원하기에 최적의 터입니다."},수:{env:"💧 북쪽 · 계곡·고지",season:"겨울(12~2월) 방문 최적",color:"#38BDF8",r:"56,189,248",detail:"깊고 고요한 수(水) 기운이 흐르는 도량입니다. 북쪽 음기 속에서 집중력과 통찰력이 한층 깊어지며, 학업·시험·지혜·직관을 기원하는 기도에 가장 강한 터입니다."}},G=ve[$.templeOhaeng],c=G?'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;"><div style="background:rgba('+G.r+",0.10);border:1px solid rgba("+G.r+',0.3);border-radius:10px;padding:10px 12px;"><div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:4px;">자연 환경</div><div style="font-size:13px;font-weight:800;color:'+G.color+';">'+G.env+'</div></div><div style="background:rgba('+G.r+",0.10);border:1px solid rgba("+G.r+',0.3);border-radius:10px;padding:10px 12px;"><div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:4px;">최적 방문 시기</div><div style="font-size:13px;font-weight:800;color:'+G.color+';">'+G.season+'</div></div></div><div style="font-size:13px;color:#CBD5E1;line-height:1.8;">'+G.detail+"</div>":'<p style="margin:0;">'+b[$.templeOhaeng]+"</p>";i+=S(j.ohaeng,k+" "+$.templeOhaeng+"(오행) 기운 · 환경",c)}if(e.reason&&(i+=S(j.reason,"✨ 나와의 인연 이유",'<p style="margin:0;">'+e.reason+"</p>")),F){const r=j.purpose;i+='<div class="ds-card" style="background:rgba('+r.r+",0.06);border:1px solid rgba("+r.r+",0.22);border-left:4px solid "+r.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+r.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+r.r+',0.2);">🎯 기도 목적</div>',i+='<span style="display:inline-block;background:rgba('+r.r+",0.18);border:1.5px solid rgba("+r.r+",0.5);color:"+r.c+';border-radius:24px;padding:9px 26px;font-weight:800;font-size:16px;">'+F+"</span>",i+="</div>"}if(Array.isArray(C)&&C.length){var h='<div style="display:flex;flex-direction:column;gap:12px;">'+C.map(function(r,d){var x=r.split(":"),M=x.length>1?x[0]+":":"",v=x.length>1?x.slice(1).join(":").trim():r;return'<div style="display:flex;gap:10px;align-items:flex-start;"><span style="flex:0 0 auto;width:24px;height:24px;border-radius:50%;background:rgba(52,211,153,0.18);border:1.5px solid rgba(52,211,153,0.4);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;color:#34D399;margin-top:1px;">'+(d+1)+'</span><div style="flex:1;">'+(M?'<span style="color:#34D399;font-weight:800;">'+M+"</span> ":"")+'<span style="color:#E2E8F0;">'+v+"</span></div></div>"}).join("")+"</div>";i+=S(j.guide,"🙏 이렇게 기도해보세요",h)}(function(){var r=$.templeOhaeng||"금",d=a&&a.targetOhaeng||a&&a.weak&&a.weak.부족오행||r,x=$.name||"",M=$.history||"",v=x+" "+M,g={대웅전:!0};/관음/.test(x)&&(g.관음전=!0),/약사/.test(x)&&(g.약사전=!0),/지장/.test(x)&&(g.지장전=!0),/나한/.test(x)&&(g.나한전=!0),/미타|극락/.test(x)&&(g.극락전=!0),/문수/.test(x)&&(g.문수전=!0),/비로|화엄/.test(x)&&(g.비로전=!0),/관음도량|관음전/.test(v)&&(g.관음전=!0),/약사전|약사여래/.test(v)&&(g.약사전=!0),/지장전|지장보살/.test(v)&&(g.지장전=!0),/나한전|오백나한/.test(v)&&(g.나한전=!0),/극락전|미타전|아미타/.test(v)&&(g.극락전=!0),/문수전|문수보살/.test(v)&&(g.문수전=!0),/총림|본사|대가람|대사찰|대본사/.test(v)&&(g.관음전=!0,g.약사전=!0,g.나한전=!0,g.지장전=!0);var P={대웅전:{icon:"🏛️",color:"#FBBF24"},관음전:{icon:"🌸",color:"#F472B6"},약사전:{icon:"💊",color:"#34D399"},나한전:{icon:"🙏",color:"#FB923C"},지장전:{icon:"🕯️",color:"#94A3B8"},극락전:{icon:"🌅",color:"#818CF8"},문수전:{icon:"📚",color:"#38BDF8"},비로전:{icon:"☀️",color:"#FCD34D"}},X={재물운:{must:["대웅전"],want:["나한전","관음전","지장전"]},건강운:{must:["대웅전"],want:["약사전","관음전","나한전"]},학업운:{must:["대웅전"],want:["문수전","나한전","관음전"]},인연운:{must:["대웅전"],want:["관음전","나한전","지장전"]},가정운:{must:["대웅전"],want:["관음전","지장전","나한전"]},수험합격:{must:["대웅전"],want:["나한전","문수전","관음전"]},취업운:{must:["대웅전"],want:["나한전","관음전","지장전"]},출산기도:{must:["대웅전"],want:["관음전","약사전","지장전"]}},K={재물운:"금",건강운:"목",학업운:"수",인연운:"화",가정운:"토",수험합격:"수",취업운:"금",출산기도:"목"},q={수:"관음전",목:"약사전",화:"관음전",토:"지장전",금:"나한전"},O=X[F];if(!O)return;var ce=K[F]||r,Y=[q[d],q[ce],q[r]].concat(O.want).filter(function(le,ke,U){return le&&U.indexOf(le)===ke}),N=[],ie=[];Y.forEach(function(le){g[le]?N.push(le):ie.push(le)});var ae=O.must,be=N.filter(function(le){return ae.indexOf(le)<0}),je=ae.concat(be.slice(0,1)),ye=be.slice(1).concat(ie).slice(0,2),we={대웅전:{재물운:'석가모니 부처님께 <strong style="color:#FBBF24;">삼배</strong>를 올리며 사업·재물의 번창을 발원합니다.',건강운:'나와 가족의 건강을 위해 <strong style="color:#FBBF24;">삼배</strong>와 함께 간절히 발원합니다.',학업운:'집중력·기억력이 높아지기를 <strong style="color:#FBBF24;">삼배</strong>로 발원합니다.',인연운:'좋은 인연이 이루어지도록 <strong style="color:#FBBF24;">삼배</strong>와 함께 진솔하게 발원합니다.',가정운:'가족 이름을 마음에 품고 <strong style="color:#FBBF24;">삼배</strong>로 가정의 평안을 발원합니다.',수험합격:'시험 날짜와 이름을 소원지에 적어 <strong style="color:#FBBF24;">삼배</strong>와 함께 올립니다.',취업운:'원하는 직장을 구체적으로 떠올리며 <strong style="color:#FBBF24;">삼배</strong>를 올립니다.',출산기도:'새 생명의 건강한 탄생을 위해 <strong style="color:#FBBF24;">삼배</strong>와 함께 발원합니다.'},관음전:{재물운:"관세음보살님께 자비로운 풍요와 기회를 열어달라고 발원합니다.",건강운:"관세음보살님의 치유 자비력으로 몸과 마음의 회복을 빕니다.",학업운:"관세음보살님께 지혜의 문이 열리기를 간청합니다.",인연운:"관음보살님은 인연의 보살. 좋은 만남이 이루어지도록 발원하세요.",가정운:"가족 모두의 건강과 화목을 관세음보살님께 빕니다.",수험합격:"시험장에서 침착하고 자신 있게 실력을 발휘할 수 있도록 빕니다.",취업운:"좋은 직장과의 인연이 열리도록 관세음보살님께 발원합니다.",출산기도:"관음보살님은 산모와 아기를 보살펴 주시는 분. 정성껏 발원하세요."},약사전:{재물운:"약사여래님의 지혜 기운으로 재물이 바르게 쌓이기를 발원합니다.",건강운:'약사여래 부처님께 <strong style="color:#34D399;">질병 치유와 건강 회복</strong>을 간절히 발원합니다.',학업운:"약사여래님의 맑은 기운으로 머리가 밝아지기를 빕니다.",인연운:"몸과 마음이 건강해야 좋은 인연도 옵니다. 건강 발원 후 인연을 비세요.",가정운:"가족 모두의 건강을 약사여래 부처님께 발원합니다.",수험합격:"시험 당일 컨디션이 최상이 되기를 약사여래님께 빕니다.",취업운:"건강한 몸과 마음으로 취업 준비에 임할 수 있도록 발원합니다.",출산기도:"산모와 아기의 건강을 약사여래 부처님께 특별히 발원하세요."},나한전:{재물운:"오백 나한님 중 내 소원을 들어줄 나한을 찾아 재물운을 빕니다.",건강운:"나한님께 나와 가족의 회복을 소원합니다. 내 이름을 불러주세요.",학업운:"오백 나한님 중 학업을 이뤄준 나한을 찾아 합격을 발원합니다.",인연운:"나한님께 좋은 인연을 맺어달라는 소원을 올립니다.",가정운:"가정의 평화를 이루어줄 나한님께 소원을 올립니다.",수험합격:"오백 나한님 중 합격을 도운 나한을 찾아 간절히 소원을 올립니다.",취업운:"취업의 기회를 열어줄 나한님께 소원을 올립니다.",출산기도:"새 생명을 맞이할 수 있도록 나한님께 소원을 올립니다."},지장전:{재물운:"지장보살님께 사업의 기반을 튼튼히 해달라고 발원합니다.",건강운:"조상과 가족의 업장을 소멸하여 건강이 회복되기를 빕니다.",학업운:"조상님의 가호로 학업이 이루어지기를 지장보살님께 발원합니다.",인연운:"인연의 장애를 풀어달라고 지장보살님께 발원합니다.",가정운:"조상과 가족 모두가 평안하도록 지장보살님께 발원합니다.",수험합격:"합격을 가로막는 업장이 소멸되기를 지장보살님께 빕니다.",취업운:"취업을 막는 장애가 사라지도록 지장보살님께 발원합니다.",출산기도:"순산과 아이의 안녕을 지장보살님께 발원합니다."},극락전:{재물운:"아미타 부처님의 자비 원력으로 풍요로운 삶을 발원합니다.",건강운:"아미타 부처님께 건강과 장수를 발원합니다.",학업운:"아미타 부처님의 지혜 광명이 학업에 비추기를 발원합니다.",인연운:"아미타 부처님의 원력으로 좋은 인연이 이루어지기를 빕니다.",가정운:"아미타 부처님께 가정의 평화와 행복을 발원합니다.",수험합격:"아미타 부처님의 원력으로 시험에서 실력 발휘를 발원합니다.",취업운:"아미타 부처님의 자비 원력으로 취업의 기회가 열리기를 빕니다.",출산기도:"아미타 부처님께 새 생명의 탄생을 발원합니다."},문수전:{학업운:"지혜의 문수보살님께 학업 성취와 총명함을 간절히 발원합니다.",수험합격:"문수보살님은 지혜의 상징. 시험 합격을 간절히 발원하세요.",취업운:"문수보살님의 지혜로 취업의 방향이 열리기를 발원합니다.",재물운:"문수보살님의 지혜로 바른 재물운이 열리기를 발원합니다.",건강운:"문수보살님의 지혜 기운으로 건강이 회복되기를 빕니다.",인연운:"지혜로운 문수보살님께 좋은 인연이 이루어지기를 발원합니다.",가정운:"문수보살님의 지혜로 가정이 화목해지기를 발원합니다.",출산기도:"문수보살님의 지혜 기운으로 건강한 아이가 태어나기를 빕니다."}};function Ce(le,ke){var U=P[le]||{icon:"🏯",color:"#94A3B8"},xe=we[le]||{},Be=xe[F]||le+"에서 정성껏 기도를 올립니다.",ze=ke?'<span style="font-size:10px;background:rgba(148,163,184,0.2);color:#94A3B8;border-radius:4px;padding:2px 6px;margin-left:6px;vertical-align:middle;">있는 경우</span>':"";return'<div style="display:flex;gap:12px;align-items:flex-start;background:rgba(255,255,255,0.04);border-radius:12px;padding:12px 14px;border:1px solid '+(ke?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.1)")+";opacity:"+(ke?"0.7":"1")+';"><div style="flex:0 0 auto;width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,0.07);display:flex;align-items:center;justify-content:center;font-size:20px;">'+U.icon+'</div><div style="flex:1;"><div style="font-size:14px;font-weight:800;color:'+U.color+';margin-bottom:4px;">'+le+ze+'</div><div style="font-size:13px;color:#CBD5E1;line-height:1.6;">'+Be+"</div></div></div>"}var $e='<div style="display:flex;flex-direction:column;gap:10px;">';je.forEach(function(le){$e+=Ce(le,!1)}),ye.forEach(function(le){$e+=Ce(le,!0)}),$e+="</div>",i+=S({r:"251,191,36",c:"#FBBF24"},"🏯 방문할 전각 안내",$e)})(),function(){var r=[{icon:"🙏",label:"삼배 방법",color:"#34D399",desc:'<strong style="color:#34D399;">합장(두 손 모음)</strong> 후 무릎을 꿇고 이마를 바닥에 댑니다. 총 세 번 반복 — 불(佛)·법(法)·승(僧) 삼보에 귀의하는 예입니다.'},{icon:"👘",label:"복장",color:"#818CF8",desc:'<strong style="color:#818CF8;">단정한 복장</strong>을 권장합니다. 민소매·짧은 반바지는 피하고, 실내 전각 입장 시 모자를 벗으세요.'},{icon:"🤫",label:"경내 예절",color:"#38BDF8",desc:'<strong style="color:#38BDF8;">사진 촬영</strong>은 스님이나 안내판을 확인 후 허용된 곳에서만. 목소리를 낮추고 뛰지 않습니다.'},{icon:"🧎",label:"스님 만남",color:"#F472B6",desc:'스님께는 <strong style="color:#F472B6;">합장 반배(고개를 30° 숙임)</strong>로 인사드립니다. 악수보다 합장이 예의입니다.'},{icon:"🕯️",label:"소원지·불전",color:"#FBBF24",desc:'<strong style="color:#FBBF24;">소원지</strong>는 이름·생년월일·소원을 적어 담당 스님 혹은 지정함에 제출합니다. 불전함(헌금)은 자유입니다.'}],d=[{icon:"🌅",label:"새벽 예불",color:"#FB923C",desc:'보통 <strong style="color:#FB923C;">새벽 4~5시</strong>에 진행됩니다. 일반인도 조용히 참관 가능하며, 하루 중 가장 청정한 기운을 받을 수 있습니다.'},{icon:"🍚",label:"사찰 공양",color:"#34D399",desc:'일부 사찰에서 <strong style="color:#34D399;">점심 공양(무료 또는 소액)</strong>을 제공합니다. 방문 전 전화로 확인하고 감사한 마음으로 참여하세요.'},{icon:"📿",label:"108배",color:"#818CF8",desc:'<strong style="color:#818CF8;">108번의 절</strong>은 108가지 번뇌를 끊는 수행입니다. 법당 한편에서 매트를 빌려 할 수 있는 사찰도 있습니다.'},{icon:"🌿",label:"템플스테이",color:"#38BDF8",desc:'1박 이상 머물며 스님과 함께 생활하는 <strong style="color:#38BDF8;">템플스테이</strong>를 운영하는 사찰이 많습니다. 한국불교문화사업단 앱에서 예약 가능합니다.'}],x='<div style="display:flex;flex-direction:column;gap:10px;">'+r.map(function(v){return'<div style="display:flex;gap:10px;align-items:flex-start;"><div style="flex:0 0 auto;width:32px;height:32px;border-radius:8px;background:rgba(255,255,255,0.06);display:flex;align-items:center;justify-content:center;font-size:16px;margin-top:1px;">'+v.icon+'</div><div style="flex:1;"><div style="font-size:13px;font-weight:800;color:'+v.color+';margin-bottom:3px;">'+v.label+'</div><div style="font-size:13px;color:#CBD5E1;line-height:1.6;">'+v.desc+"</div></div></div>"}).join("")+"</div>",M='<div style="display:flex;flex-direction:column;gap:10px;">'+d.map(function(v){return'<div style="display:flex;gap:10px;align-items:flex-start;"><div style="flex:0 0 auto;width:32px;height:32px;border-radius:8px;background:rgba(255,255,255,0.06);display:flex;align-items:center;justify-content:center;font-size:16px;margin-top:1px;">'+v.icon+'</div><div style="flex:1;"><div style="font-size:13px;font-weight:800;color:'+v.color+';margin-bottom:3px;">'+v.label+'</div><div style="font-size:13px;color:#CBD5E1;line-height:1.6;">'+v.desc+"</div></div></div>"}).join("")+"</div>";i+=S({r:"52,211,153",c:"#34D399"},"🙏 기도 예절",x),i+=S({r:"251,191,36",c:"#FBBF24"},"💡 방문 팁",M)}();const B=F?{재물운:function(r){return r+` 삼보님께 귀의합니다.

자비하신 부처님, 오늘 저는 `+r+`의 금(金) 기운이 가득한 이 도량에서 간절히 발원하오니,
...
나무아미타불 관세음보살.`},건강운:function(r){return r+` 삼보님께 귀의합니다.

자비하신 약사여래 부처님, 오늘 저는 `+r+`의 토(土) 기운이 깃든 이 청정 도량에서 발원하오니,
몸과 마음의 모든 병이 사라지고 건강한 기운이 충만하게 하여 주시옵소서.
바른 음식, 바른 생각, 바른 쉼으로 이 몸을 아끼고 사랑하여
오래도록 가족 곁에 있게 하여 주시옵소서.

나무약사유리광여래.`},학업운:function(r){return r+` 삼보님께 귀의합니다.

지혜의 문수보살님, 오늘 저는 `+r+`의 수(水) 기운이 흐르는 이 도량에서 간절히 발원하오니,
배움에 대한 집중력과 기억력이 밝아지고 깊은 지혜가 열리게 하여 주시옵소서.
목표한 결과를 이루게 하여 주시옵소서.

나무대지문수사리보살.`},인연운:function(r){return r+` 삼보님께 귀의합니다.

자비로우신 관세음보살님, 오늘 저는 `+r+`의 화(火) 기운이 충만한 이 도량에서 발원하오니,
저와 진정으로 인연이 맞는 소중한 사람과의 만남이 이루어지게 하여 주시옵소서.
서로 존중하고 아끼며 함께 성장하고 행복할 수 있는 깊은 인연을 맺게 하여 주시옵소서.

나무대자대비관세음보살.`},가정운:function(r){return r+` 삼보님께 귀의합니다.

자비하신 부처님, 오늘 저는 `+r+`의 목(木) 기운이 가득한 이 도량에서 온 가족을 위해 발원하오니,
저희 가정에 화목과 평안이 넘치게 하여 주시옵소서.
서로 배려하고 사랑하는 마음이 깊어지게 하여 주시옵소서.

나무아미타불.`},수험합격:function(r){return r+` 삼보님께 귀의합니다.

문수보살님, 오늘 저는 `+r+`의 화(火) 기운이 빛나는 이 도량에서 간절히 발원하오니,
다가오는 시험에서 저의 능력을 온전히 발휘하여 합격의 기쁨을 얻게 하여 주시옵소서.
침착하고 자신 있게 문제를 풀어낼 수 있도록 이끌어 주시옵소서.

나무석가모니불.`},취업운:function(r){return r+` 삼보님께 귀의합니다.

자비하신 부처님, 오늘 저는 `+r+`의 금(金) 기운이 빛나는 이 도량에서 발원하오니,
저의 능력과 열정을 알아봐 주는 좋은 직장과의 인연이 이루어지게 하여 주시옵소서.
취업 후에는 성실하게 일하며 성장하는 사람이 되겠습니다.

나무아미타불.`},출산기도:function(r){return r+` 삼보님께 귀의합니다.

자비하신 부처님과 칠성님께, 오늘 저는 `+r+`의 목(木) 기운이 가득한 이 도량에서 간절히 발원하오니,
건강하고 복된 새 생명이 저희 품에 안기게 하여 주시옵소서.
산모와 아기 모두 건강하게 하여 주시옵소서.

나무관세음보살.`}}[F]:null;if(B){const r=B(m.name||"이 사찰"),d=j.gido;f?(i+='<div class="ds-card" style="background:rgba('+d.r+",0.07);border:1px solid rgba("+d.r+",0.25);border-left:4px solid "+d.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+d.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+d.r+',0.2);">🕯️ 맞춤 기도문</div>',i+='<div style="font-size:14px;color:#E2E8F0;line-height:2.1;white-space:pre-line;background:rgba(0,0,0,0.2);border-radius:12px;padding:16px;">'+r+"</div>",i+='<button id="gido-copy-btn" style="margin-top:14px;display:inline-flex;align-items:center;gap:6px;background:rgba('+d.r+",0.15);border:1px solid rgba("+d.r+",0.4);color:"+d.c+';border-radius:14px;padding:9px 20px;font-size:13px;font-weight:700;cursor:pointer;">📋 기도문 복사</button>',i+='<textarea id="gido-raw" style="position:absolute;left:-9999px;opacity:0;" readonly>'+r+"</textarea>",i+="</div>"):(i+='<div class="ds-card" style="background:rgba('+d.r+",0.07);border:1px solid rgba("+d.r+",0.25);border-left:4px solid "+d.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+d.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+d.r+',0.2);">🕯️ 맞춤 기도문</div>',i+='<div style="font-size:14px;color:#94A3B8;line-height:2;background:rgba(0,0,0,0.2);border-radius:12px;padding:16px;filter:blur(2px);pointer-events:none;">'+r.slice(0,80)+"…</div>",i+='<div style="margin-top:14px;text-align:center;font-size:15px;font-weight:800;color:#1A1200;background:#FCD34D;border-radius:10px;padding:12px 16px;letter-spacing:.03em;">🔒 전체 기도문은 멤버십 전용입니다</div>',i+="</div>")}if(i+='<div id="history-section">'+(T?'<div class="ds-card" style="background:rgba(217,119,6,0.06);border:1px solid rgba(217,119,6,0.22);border-left:4px solid #D97706;border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;"><div style="font-size:14px;font-weight:800;color:#D97706;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(217,119,6,0.2);">📜 유래 · 연혁</div><div id="history-body" style="font-size:14px;color:#E2E8F0;line-height:1.9;"><p style="margin:0;">'+te+"</p></div></div>":"")+"</div>",u.length>0){const r=j.date,d=["일","월","화","수","목","금","토"],x=u.slice(0,V).map(function(M){const v=M.date.split("-"),g=parseInt(v[1]),P=parseInt(v[2]),X=new Date(parseInt(v[0]),g-1,P).getDay(),K=d[X],q=X===0||X===6,O=M.dayOhaeng&&W[M.dayOhaeng[0]]||"✶",ce=M.dayOhaeng&&t[M.dayOhaeng[0]]||t.금;return'<div class="detail-date-card" style="display:inline-flex;flex-direction:column;align-items:center;gap:5px;background:rgba('+ce.r+",0.1);border:1px solid rgba("+ce.r+',0.35);border-radius:16px;padding:14px 18px;margin:4px;min-width:74px;"><span style="font-size:20px;">'+O+'</span><span style="font-size:17px;font-weight:900;color:'+ce.c+';">'+g+"/"+P+'</span><span style="font-size:12px;color:'+(q?"#F87171":"rgba(255,255,255,0.6)")+';font-weight:600;">('+K+")</span>"+(M.dayOhaeng?'<span style="font-size:10px;color:'+ce.c+';font-weight:700;">'+Array.from(M.dayOhaeng).join("·")+"일</span>":"")+"</div>"}).join("");i+='<div class="ds-card" style="background:rgba('+r.r+",0.06);border:1px solid rgba("+r.r+",0.22);border-left:4px solid "+r.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+r.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+r.r+',0.2);">📅 추천 방문 날짜'+(f?"":'<span style="font-size:11px;font-weight:400;opacity:.45;margin-left:8px;">상위 3일 · 전체는 멤버 전용</span>')+"</div>",i+='<div style="display:flex;flex-wrap:wrap;gap:2px;">'+x+"</div>",!f&&u.length>=3&&(i+='<div style="margin-top:12px;font-size:12px;color:rgba(255,255,255,0.35);text-align:center;">🔒 멤버십 회원은 최대 15일 추천 날짜 제공</div>'),i+="</div>"}if(Object.keys(_).length>0){const r=j.dist,d=Object.entries(_).map(function([x,M]){const v=x===$.templeOhaeng,g=t[x]||t.금;return'<div style="display:inline-flex;align-items:center;gap:6px;padding:8px 18px;border-radius:24px;margin:4px;font-weight:800;font-size:14px;background:rgba('+g.r+","+(v?"0.18":"0.07")+");border:1.5px solid rgba("+g.r+","+(v?"0.55":"0.2")+");color:"+(v?g.c:"rgba(255,255,255,0.55)")+";"+(v?"box-shadow:0 0 14px rgba("+g.r+",0.25);":"")+'">'+(W[x]||"")+" "+x+" "+M+(v?" ✦":"")+"</div>"}).join("");i+='<div class="ds-card" style="background:rgba('+r.r+",0.06);border:1px solid rgba("+r.r+",0.22);border-left:4px solid "+r.c+';border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;">',i+='<div style="font-size:14px;font-weight:800;color:'+r.c+";margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba("+r.r+',0.2);">🔥 나의 사주 오행 분포</div>',i+='<div style="display:flex;flex-wrap:wrap;gap:2px;margin-bottom:10px;">'+d+"</div>";var y=a?a.weak&&a.weak.부족오행:null,ne=y&&y===$.templeOhaeng?'✦ 내 사주에서 가장 부족한 <b style="color:'+w+';">'+$.templeOhaeng+"</b> 기운을 이 사찰이 채워줍니다.":'✦ 이 사찰의 기운은 <b style="color:'+w+';">'+($.templeOhaeng||"")+"</b>으로, 선택하신 기도목적("+(F||"—")+")에 최적화된 도량입니다.";i+='<div style="font-size:12px;color:rgba(255,255,255,0.4);line-height:1.7;margin-top:6px;">'+ne+"</div>",i+="</div>"}f||(i+='<div style="background:rgba(212,175,55,0.06);border:1px solid rgba(212,175,55,0.3);border-radius:16px;padding:28px 20px;margin-bottom:12px;text-align:center;">',i+='<div style="font-size:30px;margin-bottom:10px;">🔒</div>',i+='<div style="font-size:16px;font-weight:800;color:#D4AF37;margin-bottom:8px;">멤버십 전용 콘텐츠</div>',i+='<div style="font-size:13px;color:rgba(255,255,255,0.5);margin-bottom:18px;">유래 전문 · 추천 날짜 전체 · 기도문 전체 &amp; 복사</div>',i+='<div class="member-unlock"><input type="text" id="detail-member-code-input" placeholder="멤버십 코드 입력" /><button id="detail-member-code-btn">확인</button></div>',i+="</div>"),i+='<div class="patent-notice-banner" style="margin-bottom:12px;"><div class="patent-notice-icon">⚖️</div><div class="patent-notice-body"><div class="patent-notice-title">지식재산권 안내</div><div class="patent-notice-text">본 서비스의 <strong>인연 시너지 산출 로직</strong>은 비가산 시너지 기반 지수 산출 방식을 적용한 독자 기술입니다.</div><span class="patent-num">특허 출원 중 (출원번호: 40-2026-00*****)</span></div></div>',i+="</div>",E.innerHTML=i,function(){const d=m.name;if(!d)return;const x=document.getElementById("detail-hero-img"),M=document.getElementById("detail-hero-overlay"),v=document.getElementById("detail-hero");if(!x||!M)return;function g(Y){if(!Y)return!1;var N=Y.toLowerCase();if(!/\.(jpe?g)/i.test(N))return!1;var ie=["flag","taegeuk","taeguk","portrait","map","document","signature","seal","coin","stamp","symbol","logo","icon","graph","chart","diagram","hangeul","hangul","korean_language","korea_map","location","emblem","crest","arms","건물","도로","지도","교통","버스","지하철","아파트"];return!ie.some(function(ae){return N.indexOf(ae)!==-1})}function P(Y){var N=new Image;N.onload=function(){N.naturalWidth<300||N.naturalHeight<200||(x.style.backgroundImage="url("+Y+")",x.style.opacity="1",M.style.opacity="1",v&&(v.style.background="none"),x.style.cursor="zoom-in",x.onclick=function(){var ie=document.createElement("div");ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:99999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;";var ae=new Image;ae.src=Y,ae.style.cssText="max-width:92vw;max-height:88vh;border-radius:12px;box-shadow:0 8px 48px #000;",ie.appendChild(ae),ie.onclick=function(){document.body.removeChild(ie)},document.body.appendChild(ie)})},N.src=Y}var X={목:"linear-gradient(160deg,rgba(34,85,34,0.85) 0%,rgba(20,60,20,0.95) 100%)",화:"linear-gradient(160deg,rgba(120,40,10,0.85) 0%,rgba(80,20,5,0.95) 100%)",토:"linear-gradient(160deg,rgba(90,70,20,0.85) 0%,rgba(60,45,10,0.95) 100%)",금:"linear-gradient(160deg,rgba(90,70,10,0.85) 0%,rgba(60,45,5,0.95) 100%)",수:"linear-gradient(160deg,rgba(10,40,80,0.85) 0%,rgba(5,20,60,0.95) 100%)"};function K(){var Y=$.templeOhaeng||"금",N=X[Y]||X.금;v&&(v.style.background=N)}function q(Y){return fetch("https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch="+encodeURIComponent(Y)+"&srnamespace=6&srlimit=10&srprop=&format=json&origin=*").then(function(N){return N.ok?N.json():null}).then(function(N){if(!N||!N.query||!N.query.search)return[];var ie=N.query.search.map(function(be){return be.title}).filter(function(be){return/\.jpe?g$/i.test(be)}).slice(0,8);if(!ie.length)return[];var ae=ie.map(encodeURIComponent).join("%7C");return fetch("https://commons.wikimedia.org/w/api.php?action=query&prop=imageinfo&iiprop=url%7Csize&iiurlwidth=900&titles="+ae+"&format=json&origin=*").then(function(be){return be.ok?be.json():null}).then(function(be){if(!be)return[];var je=be.query&&be.query.pages?be.query.pages:{};return Object.values(je).map(function(ye){return ye.imageinfo&&ye.imageinfo[0]}).filter(function(ye){return ye&&ye.thumburl&&g(ye.thumburl)&&(ye.width||0)>=400&&(ye.height||0)>=300})})}).catch(function(){return[]})}var O=!1;function ce(Y){return O||!Y||!Y.length?!1:(O=!0,P(Y[0].thumburl),!0)}fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=pageimages|info&pithumbsize=900&titles="+encodeURIComponent(d)+"&format=json&origin=*").then(function(Y){return Y.ok?Y.json():null}).then(function(Y){if(!O&&Y){var N=Y.query&&Y.query.pages?Y.query.pages:{},ie=Object.values(N)[0];if(!(!ie||ie.missing!==void 0)){var ae=ie.thumbnail?ie.thumbnail.source:null;ae&&g(ae)&&(O=!0,P(ae))}}}).catch(function(){}),setTimeout(function(){O||Promise.all([q(d+" 사찰"),q(d+" temple Korea")]).then(function(Y){if(!O){var N=[].concat(Y[0],Y[1]);if(!ce(N)){var ie=(m.address||"").split(" ").slice(0,2).join(" ");return ie?q(ie+" 불교 사찰"):Promise.resolve([])}}}).then(function(Y){O||!Y||ce(Y)||K()}).catch(function(){O||K()})},800)}(),function(){var d=document.getElementById("temple-gallery"),x=document.getElementById("gallery-scroll");if(!d||!x||!m.name)return;function M(g){if(!(!g||g.length===0)){var P=g.map(function(X){var K=X.thumburl,q=K.replace(/\/\d+px-/,"/1200px-");return`<div style="flex:0 0 auto;width:160px;height:110px;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,0.12);cursor:pointer;transition:transform .2s,box-shadow .2s;" onmouseover="this.style.transform='scale(1.04)';this.style.boxShadow='0 6px 20px rgba(0,0,0,.5)'" onmouseout="this.style.transform='';this.style.boxShadow=''" data-src="`+q+`" onclick="(function(u){var ov=document.createElement('div');ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.9);z-index:99999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;';var im=new Image();im.src=u;im.style.cssText='max-width:92vw;max-height:88vh;border-radius:10px;box-shadow:0 8px 40px #000;';ov.appendChild(im);ov.onclick=function(){document.body.removeChild(ov);};document.body.appendChild(ov);})(this.dataset.src)"><img src="`+K+`" style="width:100%;height:100%;object-fit:cover;" loading="lazy" onerror="this.parentElement.style.display='none'"></div>`}).join("");x.innerHTML=P,d.style.display="block"}}function v(g,P){var X=P.map(function(K){return encodeURIComponent(K)}).join("%7C");return fetch(g+"?action=query&prop=imageinfo&iiprop=url%7Csize&iiurlwidth=500&titles="+X+"&format=json&origin=*").then(function(K){return K.ok?K.json():null}).then(function(K){if(!K)return[];var q=K.query&&K.query.pages?K.query.pages:{};return Object.values(q).map(function(O){return O.imageinfo&&O.imageinfo[0]}).filter(function(O){if(!O||!O.thumburl||!/\.(jpe?g)/i.test(O.thumburl)||(O.width||0)<400||(O.height||0)<300)return!1;var ce=O.thumburl.toLowerCase(),Y=["taegeuk","taeguk","flag","portrait","map","document","signature","seal","coin","stamp","symbol","logo","icon","graph","chart","diagram","hangeul","hangul","emblem","crest","location","korea_map","korean_language","태극","지도","문서","초상","건물","도로","교통"];if(Y.some(function(ie){return ce.indexOf(ie)!==-1}))return!1;var N=(O.width||1)/(O.height||1);return!(N<.5||N>3.5)})})}fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=pageprops&titles="+encodeURIComponent(m.name)+"&format=json&origin=*").then(function(g){return g.ok?g.json():null}).then(function(g){var P=g&&g.query&&g.query.pages?g.query.pages:{},X=Object.values(P)[0],K=X&&X.pageprops&&X.pageprops.commonsCategory;return K?fetch("https://commons.wikimedia.org/w/api.php?action=query&list=categorymembers&cmtitle="+encodeURIComponent("Category:"+K)+"&cmtype=file&cmlimit=20&format=json&origin=*").then(function(q){return q.ok?q.json():null}).then(function(q){if(!q||!q.query)return null;var O=(q.query.categorymembers||[]).map(function(ce){return ce.title}).filter(function(ce){return/\.jpe?g$/i.test(ce)}).slice(0,10);return O.length?v("https://commons.wikimedia.org/w/api.php",O):null}):null}).then(function(g){return g&&g.length>=1?(M(g),Promise.resolve(!0)):fetch("https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch="+encodeURIComponent(m.name+" temple")+"&srnamespace=6&srlimit=16&srprop=&format=json&origin=*").then(function(P){return P.ok?P.json():null}).then(function(P){if(!P||!P.query||!P.query.search)return null;var X=P.query.search.map(function(K){return K.title}).filter(function(K){return/\.jpe?g$/i.test(K)}).slice(0,10);return X.length?v("https://commons.wikimedia.org/w/api.php",X):null})}).then(function(g){if(g===!0||g&&g.length>=1){g!==!0&&M(g);return}return fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=images&titles="+encodeURIComponent(m.name)+"&imlimit=20&format=json&origin=*").then(function(P){return P.ok?P.json():null}).then(function(P){if(P){var X=P.query&&P.query.pages?P.query.pages:{},K=Object.values(X)[0];if(!(!K||!K.images)){var q=K.images.map(function(O){return O.title}).filter(function(O){return/\.jpe?g$/i.test(O)}).slice(0,10);if(q.length)return v("https://ko.wikipedia.org/w/api.php",q).then(function(O){O&&O.length>=1&&M(O)})}}})}).catch(function(){})}(),function(){if(m.name){var d=document.getElementById("history-section");document.getElementById("history-body"),!(T.length>=150)&&fetch("https://ko.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=1&explaintext=1&exsentences=6&titles="+encodeURIComponent(m.name)+"&format=json&origin=*").then(function(x){return x.ok?x.json():null}).then(function(x){if(x){var M=x.query&&x.query.pages?x.query.pages:{},v=Object.values(M)[0],g=v&&v.extract?v.extract.trim():"";if(!(!g||g.length<30)&&d){var P=(T?T+`

`:"")+g,X=f?P:P.slice(0,200)+(P.length>200?'… <span style="color:rgba(255,255,255,0.35);font-size:12px;">🔒 전체보기는 멤버 전용</span>':"");d.innerHTML='<div class="ds-card" style="background:rgba(217,119,6,0.06);border:1px solid rgba(217,119,6,0.22);border-left:4px solid #D97706;border-radius:16px;padding:20px 20px 20px 22px;margin-bottom:12px;"><div style="font-size:14px;font-weight:800;color:#D97706;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(217,119,6,0.2);">📜 유래 · 연혁 <span style="font-size:11px;font-weight:400;opacity:.45;margin-left:6px;">위키백과 참조</span></div><div style="font-size:14px;color:#E2E8F0;line-height:1.9;white-space:pre-line;">'+X+"</div></div>"}}}).catch(function(){})}}();const pe=document.getElementById("gido-copy-btn"),ge=document.getElementById("gido-raw");pe&&ge&&pe.addEventListener("click",function(){navigator.clipboard.writeText(ge.value).then(function(){pe.textContent="✓ 복사됨!",setTimeout(function(){pe.innerHTML="📋 기도문 복사"},2e3)}).catch(function(){ge.select(),document.execCommand("copy")})}),(me=document.getElementById("detail-back-btn"))==null||me.addEventListener("click",()=>{var r;typeof A=="function"?A():(E.innerHTML="",E.classList.add("hidden"),p&&(p.style.display=""),(r=document.getElementById("app"))==null||r.scrollIntoView({behavior:"smooth",block:"start"}))});const ue=document.getElementById("detail-member-code-btn"),se=document.getElementById("detail-member-code-input");ue&&ue.addEventListener("click",()=>{se&&se.value.trim()===Ge?(Re(se.value.trim()),Le(e,a,!0,A)):alert("코드가 올바르지 않습니다.")})}function Pe(e){var $,n;const a=document.getElementById("results");a.classList.remove("hidden");const f=(e.results||[])[0],A=it[($=f==null?void 0:f.detail)==null?void 0:$.bearing]??0,E=_e();a.innerHTML=`
  ...
  `;const p=document.getElementById("member-code-input"),m=document.getElementById("member-code-btn");m&&m.addEventListener("click",()=>{p&&p.value.trim()===Ge?(Re(p.value.trim()),Pe(e)):alert("코드가 올바르지 않습니다.")}),document.querySelectorAll(".detail-view-btn").forEach(t=>{t.addEventListener("click",()=>{const z=parseInt(t.dataset.templeIndex);Le(e.results[z],e,E)})}),function(){var b,k,_,F;var z=(b=e.results)==null?void 0:b[0],w=((k=z==null?void 0:z.temple)==null?void 0:k.name)||"인연사찰",l=(z==null?void 0:z.score)||"",j=e.purpose||"",S="https://jamgong-inyeonsachal.vercel.app",W='🏯 나의 인연사찰은 "'+w+`"!
오행 궁합 `+l+"점 매칭 ("+j+`)

내 사주에 맞는 사찰을 찾아보세요 👇
`+S,oe="내 인연사찰은 "+w+"! — 잼공인연사찰";(_=document.getElementById("share-kakao-btn"))==null||_.addEventListener("click",function(){var C;window.Kakao&&window.Kakao.isInitialized()?window.Kakao.Share.sendDefault({objectType:"feed",content:{title:oe,description:"오행 궁합 "+l+"점 매칭 ("+j+`)
내 사주에 맞는 사찰을 찾아보세요!`,imageUrl:"https://jamgong-inyeonsachal.vercel.app/og-image.png",link:{mobileWebUrl:S,webUrl:S}},buttons:[{title:"내 인연사찰 찾기",link:{mobileWebUrl:S,webUrl:S}}]}):navigator.share?navigator.share({title:oe,text:W,url:S}).catch(function(){}):(C=navigator.clipboard)==null||C.writeText(W).then(function(){alert(`카카오톡에 붙여넣기 하세요!

`+W)})}),(F=document.getElementById("share-copy-btn"))==null||F.addEventListener("click",function(){var u;var C=document.getElementById("share-copy-btn");(u=navigator.clipboard)==null||u.writeText(S).then(function(){C&&(C.textContent="✅ 복사됨!",setTimeout(function(){C.innerHTML="🔗 링크 복사"},2e3))}).catch(function(){alert("주소: "+S)})})}(),(n=document.getElementById("demo-detail-btn"))==null||n.addEventListener("click",()=>{Le({score:94,reason:"수(水) 기운이 부족한 사주에 이 사찰의 강한 수 기운이 보완해줍니다.",temple:{name:"통도사",address:"경상남도 양산시 하북면 통도사로 108",lat:35.489166,lng:129.058611,foundedYear:646},detail:{templeOhaeng:"수",bearing:"북",distanceKm:12.3},weather:{condition:"맑음",temp:24}},{purpose:"학업운",distribution:{목:1,화:2,토:2,금:2,수:1},targetOhaeng:"수",purposeGuide:[]},!0)}),a.scrollIntoView({behavior:"smooth"})}function at(e,a){const f={목:"화",화:"토",토:"금",금:"수",수:"목"},A={목:"토",토:"수",수:"화",화:"금",금:"목"},E={목:"木",화:"火",토:"土",금:"金",수:"水"},p={목:"#64DCA0",화:"#FF6B9D",토:"#FB923C",금:"#FFB347",수:"#00D2FF"};let m,$,n,t,z,w;const l=f[e]===a,j=f[a]===e,S=A[e]===a,W=A[a]===e;return l||j?(m=88,$="상생(相生)",n="천생연분(天生緣分)",t="#00D2FF",z="0,210,255",w="오행이 서로를 생(生)해주는 가장 이상적인 조합입니다. 단, 한쪽이 지나치게 의존하면 소진될 수 있습니다."):e===a?(m=68,$="비화(比和)",n="동질형(同質型)",t="#9FE5C4",z="159,229,196",w="같은 오행으로 서로를 잘 이해하지만, 동일한 약점을 함께 지닙니다. 외부 자극 없이는 정체될 수 있습니다."):S||W?(m=42,$="상극(相剋)",n="갈등형(葛藤型)",t="#F5A623",z="245,166,35",w="오행이 서로를 극(克)합니다. 긴장과 갈등이 잦을 수 있으며, 의식적인 노력 없이는 관계가 소모적이 될 수 있습니다."):(m=62,$="중화(中和)",n="평범형(平凡型)",t="#FF6B9D",z="255,107,157",w="특별히 맞거나 부딪히는 기운이 없는 평이한 조합입니다. 큰 시너지보다는 무난한 관계가 됩니다."),{score:m,relation:$,grade:n,color:t,rgb:z,comment:w,symA:E[e]||e,symB:E[a]||a,colA:p[e]||"#fff",colB:p[a]||"#fff"}}function lt(e){var U,xe,Be,ze,o,fe,Fe,Se,Me,Oe,Ie,De;const a=document.getElementById("results");a.classList.remove("hidden");const f=_e(),A=((Be=(xe=(U=e.pillarsA)==null?void 0:U[2])==null?void 0:xe.wx)==null?void 0:Be[0])||e.targetA,E=((fe=(o=(ze=e.pillarsB)==null?void 0:ze[2])==null?void 0:o.wx)==null?void 0:fe[0])||e.targetB,p=at(A,E),m=(e.hapChung||[]).filter(s=>s.positive).length,$=(e.hapChung||[]).filter(s=>!s.positive).length,n=m*4-$*7,t=Math.max(10,Math.min(99,p.score+n)),z=Math.round(t);window._sajuContext=null,window._gunghamContext={pillarsA:e.pillarsA,pillarsB:e.pillarsB,hapChung:e.hapChung,distributionA:e.distributionA,distributionB:e.distributionB,genderA:e.genderA,genderB:e.genderB,targetA:A,targetB:E,finalScore:t,relation:p.relation,grade:p.grade};const w={목:"#64DCA0",화:"#FF6B9D",토:"#FB923C",금:"#FFB347",수:"#00D2FF"},l={목:"木",화:"火",토:"土",금:"金",수:"水"};function j(s,D,R){return!s||s.length===0?"":`
    <div class="gh-pillar-col">
      ...
      <table class="gh-saju-table">
        ...
        <tbody>
          <tr class="gh-row-stem">${s.map(Q=>{var he;return`<td style="color:${w[(he=Q.wx)==null?void 0:he[0]]||"#fff"}">${Q.stem||""}</td>`}).join("")}</tr>
          <tr class="gh-row-branch">${s.map(Q=>{var he,Ee;return`<td style="color:${w[((he=Q.wx)==null?void 0:he[1])||((Ee=Q.wx)==null?void 0:Ee[0])]||"rgba(255,255,255,0.6)"}">${Q.branch||""}</td>`}).join("")}</tr>
          <tr class="gh-row-wx">${s.map(Q=>`<td>${(Q.wx||[]).map(de=>`<span style="color:${w[de]||"#fff"}">${l[de]||de}</span>`).join("")}</td>`).join("")}</tr>
        </tbody>
      </table>
    </div>`}const S={甲己:{name:"중정합(中正合)",wx:"土",desc:"안정적이고 현실적인 결합입니다. 서로를 믿고 의지하는 관계로 장기적인 신뢰가 형성됩니다."},乙庚:{name:"인의합(仁義合)",wx:"金",desc:"원칙과 의리로 맺어진 관계입니다. 한 번 인연이 맺어지면 쉽게 끊어지지 않습니다."},丙辛:{name:"위제합(威制合)",wx:"水",desc:"강렬한 끌림의 결합입니다. 서로에게 매력을 느끼고 지략과 감각이 맞닿는 관계입니다."},丁壬:{name:"인수합(仁壽合)",wx:"木",desc:"인정 넘치고 창의적인 결합입니다. 감성이 잘 맞아 함께 성장하는 인연입니다."},戊癸:{name:"무정합(無情合)",wx:"火",desc:"열정적이고 활기찬 결합입니다. 역동적인 에너지를 서로에게서 끌어냅니다."}},W={子丑:{desc:"자축합(子丑合)으로 土의 기운이 생깁니다. 안정적이고 묵묵하게 함께하는 관계입니다."},寅亥:{desc:"인해합(寅亥合)으로 木의 기운이 생깁니다. 서로 성장을 돕는 따뜻한 인연입니다."},卯戌:{desc:"묘술합(卯戌合)으로 火의 기운이 생깁니다. 열정적인 감정의 교류가 있는 관계입니다."},辰酉:{desc:"진유합(辰酉合)으로 金의 기운이 생깁니다. 단단하고 현실적인 결합입니다."},巳申:{desc:"사신합(巳申合)으로 水의 기운이 생깁니다. 지략과 변화 속에서 교류하는 인연입니다."},午未:{desc:"오미합(午未合)으로 태양의 조화입니다. 가장 자연스럽고 따뜻한 결합 중 하나입니다."}},oe={甲庚:"갑경충(甲庚沖)입니다. 추진 방향이 정반대여서 충돌이 발생합니다. 한쪽이 밀면 다른 쪽이 버티는 구조입니다.",乙辛:"을신충(乙辛沖)입니다. 섬세한 감성과 날카로운 이성이 부딪힙니다. 표현 방식의 차이에서 갈등이 생깁니다.",丙壬:"병임충(丙壬沖)입니다. 화(火)와 수(水)의 충돌로 열정과 냉정이 부딪힙니다. 감정 기복이 심해질 수 있습니다.",丁癸:"정계충(丁癸沖)입니다. 감성과 직관이 서로를 견제합니다. 마음 속을 잘 드러내지 않아 오해가 생길 수 있습니다.",子午:"자오충(子午沖)입니다. 수(水)와 화(火)의 강렬한 충돌입니다. 감정적으로 과격해지기 쉽고 회복이 오래 걸립니다.",丑未:"축미충(丑未沖)입니다. 두 토(土)의 충돌로 고집이 부딪히는 구조입니다. 서로 자기 방식을 고수하려 합니다.",寅申:"인신충(寅申沖)입니다. 목(木)과 금(金)의 충돌입니다. 자유와 통제 사이에서 갈등이 자주 발생합니다.",卯酉:"묘유충(卯酉沖)입니다. 감성과 이성의 충돌입니다. 서로의 접근 방식이 달라 이해하는 데 시간이 필요합니다.",辰戌:"진술충(辰戌沖)입니다. 토(土) 간의 강한 충돌입니다. 양쪽 모두 쉽게 물러서지 않아 합의가 어렵습니다.",巳亥:"사해충(巳亥沖)입니다. 금(金)과 수(水) 사이의 충돌입니다. 방향과 가치관이 달라 이견이 자주 나타납니다."},b={甲:"목",乙:"목",丙:"화",丁:"화",戊:"토",己:"토",庚:"금",辛:"금",壬:"수",癸:"수"},k={甲:"양",丙:"양",戊:"양",庚:"양",壬:"양",乙:"음",丁:"음",己:"음",辛:"음",癸:"음"},_={목:"화",화:"토",토:"금",금:"수",수:"목"},F={목:"토",화:"금",토:"수",금:"목",수:"화"},C={비견:{name:"비견(比肩)",desc:"같은 오행·같은 음양의 동지입니다. 서로를 너무 잘 알아 숨길 수 없지만, 경쟁심 없이 함께 걸어갈 수 있습니다."},겁재:{name:"겁재(劫財)",desc:"같은 오행이지만 음양이 달라 미묘하게 다릅니다. 서로 자극을 주고 경쟁심도 있지만, 그만큼 활기차고 역동적인 관계입니다."},식신:{name:"식신(食神)",desc:"나의 에너지가 자연스럽게 상대방으로 흘러갑니다. 내가 더 주는 역할을 맡는 편이지만, 편안하고 풍요로운 관계입니다."},상관:{name:"상관(傷官)",desc:"창의적이고 자유로운 에너지의 교류입니다. 틀에 얽매이지 않고 서로를 변화시키는 자극적인 관계입니다."},편인:{name:"편인(偏印)",desc:"상대방의 에너지가 일방적으로 나에게 흘러옵니다. 상대방이 더 지원해주지만, 의존이 깊어지면 주도성을 잃을 수 있습니다."},정인:{name:"정인(正印)",desc:"상대방이 안정적으로 나를 성장시켜줍니다. 스승과 같은 든든하고 신뢰감 있는 관계입니다."},편재:{name:"편재(偏財)",desc:"내가 상대방을 이끌고 활용하는 관계입니다. 능동적이고 주도적으로 관계를 이끌어갑니다."},정재:{name:"정재(正財)",desc:"안정적이고 현실적인 관계입니다. 서로에게 실질적으로 이로운 신뢰의 인연입니다."},편관:{name:"편관(偏官)",desc:"상대방이 나를 압박하는 구조입니다. 긴장감이 있지만 그것이 성장의 동력이 되기도 합니다."},정관:{name:"정관(正官)",desc:"상대방이 나를 올바르게 이끌어줍니다. 격식 있고 안정적인, 사회적으로도 인정받는 관계입니다."}},u=(Se=(Fe=e.pillarsA)==null?void 0:Fe[2])==null?void 0:Se.stem,V=(Oe=(Me=e.pillarsB)==null?void 0:Me[2])==null?void 0:Oe.stem,T=(s,D)=>{const R=b[s],J=b[D];if(!R||!J)return null;const Q=k[s]===k[D];return R===J?C[Q?"비견":"겁재"]:_[R]===J?C[Q?"식신":"상관"]:_[J]===R?C[Q?"편인":"정인"]:F[R]===J?C[Q?"편재":"정재"]:F[J]===R?C[Q?"편관":"정관"]:null},te=T(u,V),Z=T(V,u),H=s=>{const D={};return s.forEach(R=>{const J=(R.type||"").split("(")[0].split("（")[0].trim(),Q=[String(R.a||""),String(R.b||"")].sort().join(""),de=`${J}|${Q}`;D[de]||(D[de]={...R,pillars:[]});const he=(R.pillarA||"").slice(0,2),Ee=(R.pillarB||"").slice(0,2);(he||Ee)&&D[de].pillars.push(`${he}·${Ee}`)}),Object.values(D)},I=H((e.hapChung||[]).filter(s=>s.positive)),i=H((e.hapChung||[]).filter(s=>!s.positive));I.length,i.length;const ee=s=>{var J;const D=[s.a,s.b].sort().join("");let R="";if(s.type.includes("천간합")){const Q=S[D]||S[[s.b,s.a].join("")];Q&&(R=`<div class="gh-hap-desc">${Q.name} · ${Q.wx}의 기운 — ${Q.desc}</div>`)}else if(s.type.includes("지지육합")){const Q=[s.a,s.b].sort().join("");Object.values(W).find((he,Ee,He)=>Object.keys(He).find(Te=>Te.split("").sort().join("")===Q.split("").sort().join("")));const de=Object.entries(W).find(([he])=>he.split("").sort().join("")===Q.split("").sort().join(""));de&&(R=`<div class="gh-hap-desc">${de[1].desc}</div>`)}else s.type.includes("삼합")&&(R='<div class="gh-hap-desc">삼합(三合)은 세 지지가 모여 강한 오행의 국(局)을 이루는 특수한 인연입니다. 두 분 사이에 운명적 연결고리가 있습니다.</div>');return`<div class="gh-hap-item positive">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        <span class="gh-hap-type">${s.type}</span>
        <span class="gh-hap-chars">${s.a} ↔ ${s.b}</span>
        ${(J=s.pillars)!=null&&J.length?`<span class="gh-hap-pill">${s.pillars.join(" / ")}</span>`:""}
      </div>
      ${R}
    </div>`},re=s=>{var J;const D=[s.a,s.b].sort().join(""),R=oe[D]||oe[[s.b,s.a].join("")]||"";return`<div class="gh-hap-item negative">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        <span class="gh-hap-type">${s.type}</span>
        <span class="gh-hap-chars">${s.a} ↔ ${s.b}</span>
        ${(J=s.pillars)!=null&&J.length?`<span class="gh-hap-pill">${s.pillars.join(" / ")}</span>`:""}
    ...
    </div>`:"",G=e.distributionA||{},c=e.distributionB||{},h=["목","화","토","금","수"],L={목:"木(목)",화:"火(화)",토:"土(토)",금:"金(금)",수:"水(수)"},B={목:"창의·성장·인자함",화:"열정·표현·감수성",토:"안정·신뢰·중재력",금:"결단·원칙·날카로움",수:"지혜·직관·유연성"},y={};I.forEach(s=>{const D=[s.a,s.b].sort().join("");if((s.type||"").includes("천간합")){const R=S[D]||S[[s.b,s.a].join("")];R!=null&&R.wx&&(y[R.wx]=(y[R.wx]||0)+1)}});const ne={목:"창의·성장",화:"정(情)·열정·인연",토:"안정·신뢰",금:"의리·결속",수:"지혜·유연성"},pe=Object.entries(y).sort((s,D)=>D[1]-s[1]),ge=pe.length>0?"✨ 합화오행: "+pe.map(([s,D])=>`<strong style="color:${w[s]||"#fff"}">${L[s]||s}(${ne[s]})</strong>${D>1?` ×${D}`:""}`).join(" · ")+" 기운이 두 분 사이에서 생성됩니다.":"",ue=te?`
    <div class="gh-card">
      <div class="gh-section">
    ...
    </div>`:"",se={};h.forEach(s=>{se[s]=(G[s]||0)+(c[s]||0)});const me=[...h].sort((s,D)=>(se[s]||0)-(se[D]||0)),r=me[0],d=me[me.length-1],x=h.filter(s=>Math.abs((G[s]||0)-(c[s]||0))>=2);let M="";if(r&&se[r]<=1&&(M+=`<div class="gh-dist-interp">⚠ 두 분 모두 <strong style="color:${w[r]}">${L[r]}</strong>(${B[r]})이 부족합니다. 이 기운이 약한 영역에서 함께 어려움을 겪을 수 있습니다.</div>`),x.length>0){const s=x[0],D=(G[s]||0)>(c[s]||0);M+=`<div class="gh-dist-interp good">✓ <strong style="color:${w[s]}">${L[s]}</strong>에서 한 분이 강하고 다른 분이 약합니다. ${D?"나":"상대방"}의 ${L[s]} 기운이 ${D?"상대방":"나"}를 채워주는 긍정적인 보완 관계입니다.</div>`}se[d]>=6&&(M+=`<div class="gh-dist-interp warn">△ 두 분 합산 <strong style="color:${w[d]}">${L[d]}</strong>이 과도합니다. 이 오행의 특성(${B[d]})이 지나치게 강해 균형을 잃을 수 있습니다.</div>`);const v=new Set(["甲","丙","戊","庚","壬"]),g=new Set(["乙","丁","己","辛","癸"]),P=new Set(["子","寅","辰","午","申","戌"]),X=new Set(["丑","卯","巳","未","酉","亥"]),K=s=>{let D=0,R=0,J=0,Q=0;return s.forEach(de=>{v.has(de.stem)?D++:g.has(de.stem)&&R++,P.has(de.branch)?J++:X.has(de.branch)&&Q++}),{yangStem:D,yinStem:R,yangBranch:J,yinBranch:Q}},q=K(e.pillarsA||[]),O=K(e.pillarsB||[]),ce=q.yangStem+q.yangBranch,Y=q.yinStem+q.yinBranch,N=O.yangStem+O.yangBranch,ie=O.yinStem+O.yinBranch,ae=(()=>{const s=ce>Y?"양(陽)형":ce<Y?"음(陰)형":"균형형",D=N>ie?"양(陽)형":N<ie?"음(陰)형":"균형형",R={"양(陽)형":"활동적이고 외향적입니다. 먼저 나서고 에너지를 발산하는 편입니다.","음(陰)형":"내성적이고 수용적입니다. 관찰하고 흡수하며 내면이 깊습니다.",균형형:"음양이 고르게 분포되어 상황에 따라 유연하게 대처합니다."};let J="";return s!==D?J="서로 다른 음양 기질이 만나 자연스러운 끌림이 생깁니다. 보완 관계가 형성됩니다.":s==="양(陽)형"?J="두 분 모두 적극적이라 활기차지만 주도권 갈등이 생길 수 있습니다.":s==="음(陰)형"?J="두 분 모두 내성적이라 서로를 깊이 이해하지만 먼저 나서는 것을 꺼릴 수 있습니다.":J="두 분 모두 균형 잡힌 기질로 상황 적응력이 좋습니다.",{typeA:s,typeB:D,descA:R[s],descB:R[D],chem:J}})(),be=`
    <div class="gh-section">
      <div class="gh-section-title">🌊 오행 분포 비교</div>
      <div class="gh-dist-grid">
        ${h.map(s=>{const D=G[s]||0,R=c[s]||0,J=w[s]||"#fff";return`
          <div class="gh-dist-row">
            <span class="gh-dist-name" style="color:${J}">${l[s]||s}(${s})</span>
            <div class="gh-dist-bars">
              <div class="gh-bar-wrap">
                <div class="gh-bar gh-bar-a" style="width:${D/8*100}%;background:${J}"></div>
              </div>
    ...
    <div style="margin-bottom:10px;padding:10px 13px;background:rgba(255,255,255,0.04);border-radius:10px;border:1px solid rgba(255,255,255,0.08);font-size:12.5px;line-height:1.7;color:rgba(255,255,255,0.7);">
      ...
      <div class="gh-advice-group">
        ...
        ${we.strength.map(s=>`<div class="gh-advice-item positive">✓ ${s}</div>`).join("")}
      </div>
      <div class="gh-advice-group" style="margin-top:10px">
        <div class="gh-advice-label caution">주의 요소</div>
        ${we.caution.map(s=>`<div class="gh-advice-item caution">⚠ ${s}</div>`).join("")}
      </div>
    </div>`;a.innerHTML=`
    <!-- 홈 버튼 -->
    <div class="detail-nav-row" style="margin-bottom:16px;">
    ...
    <div class="gh-card">
      <div class="gh-section">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;margin-top:8px;">
          ...
          ${J&&Q&&D===R?'<div style="margin-top:4px;padding:8px 12px;background:rgba(255,215,0,0.07);border-radius:8px;font-size:12px;color:rgba(255,215,0,0.8);">✨ 두 분이 같은 일주(同日柱)입니다. 매우 드문 인연으로, 서로를 거울처럼 반추하는 관계입니다.</div>':""}
        </div>
      </div>
    </div>`})():""}
